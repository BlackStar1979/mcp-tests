...
Every directory containing tracked project files has a `DIRECTORY.md` map guarded by the repo-wide audit. Regenerate deterministically with `npm run docs:directory` and verify without mutation through `node scripts/generate_directory_docs.js --check`.
