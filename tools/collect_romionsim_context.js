...
const { collectRomionsimContext, retrievalErrorMetadata } = require("../src/util/workspace_index");
...
const collectRomionsimContextTool = {
  ...
  async execute(args = {}) {
    try {
      return await collectRomionsimContext(args.query, {
        limit: args.limit,
      ...
      return {
        ...
        ...retrievalErrorMetadata(args.path, "romionsim"),
        count: 0,
        files: [],
      };
    }
  ...
  summarizeArgs(args = {}) {
    return {
      query_length_chars: String(args.query || "").length,
      limit: Number(args.limit || 12),
      include_pinned: args.include_pinned !== false,
      path: String(args.path || "romionsim"),
    };
  },
