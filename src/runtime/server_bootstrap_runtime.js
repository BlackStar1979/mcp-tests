...
function runServerBootstrapRuntime({ argv = process.argv, env = process.env, rootDir = path.resolve(__dirname, "../..") } = {}) {
  ...
  const processIdempotencySecret = String(env.MCP_PROCESS_IDEMPOTENCY_SECRET || "");
  let legacyProcessIdempotencySecret = "";
  if (bootstrapConfig.authMode === "oauth21") {
    ...
    if (processIdempotencySecret && processIdempotencySecret === secretConfig.operatorSecret) {
      const error = new Error("MCP_PROCESS_IDEMPOTENCY_SECRET must be distinct from the OAuth21 operator secret.");
      error.code = "process_idempotency_secret_reuse";
      throw error;
    }
    // Before this separation, OAuth21 runtimes used operatorSecret as the idempotency HMAC key.
    // Keep it lookup-only for retained mappings so an upgrade cannot turn a lost-response retry
    // into duplicate execution. New mappings MUST use only processIdempotencySecret or SHA-256.
    legacyProcessIdempotencySecret = secretConfig.operatorSecret;
  ...
  if (runtimeSideEffectsEnabled) {
    ...
    getDefaultProcessJobManager({
      ...
      legacyIdempotencySecret: legacyProcessIdempotencySecret,
    });
  }
