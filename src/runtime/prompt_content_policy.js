...
const PROMPT_CONTENT_POLICY_VERSION = "mcp-tests-prompt-content-runtime-v1";
const CONTENT_BOUNDARY_KEY = "mcp-tests/contentBoundary";
const UNTRUSTED_TOOL_OUTPUT = "untrusted_tool_output";
...
function classifyToolResultContent(payload) {
  const inspection = inspectForPromptInjection(payload, { source: "tool" });
  return {
    policy_version: PROMPT_CONTENT_POLICY_VERSION,
    trust_class: UNTRUSTED_TOOL_OUTPUT,
    instruction_handling: "data_only",
    promotion_allowed: false,
    instruction_risk: inspection.severity,
    finding_count: inspection.finding_count,
    finding_ids: inspection.findings.map((item) => item.id).sort(),
  };
}

function buildModelVisibleContentBoundary(payload) {
  return {
    type: "text",
    text: JSON.stringify({
      [CONTENT_BOUNDARY_KEY]: classifyToolResultContent(payload),
    }),
  };
}

function validatePromptContentPolicy() {
  const errors = [];
  const ids = new Set();
  for (const rule of INJECTION_PATTERNS) {
    if (!rule.id || !rule.severity || !(rule.pattern instanceof RegExp)) errors.push("invalid firewall rule");
    if (ids.has(rule.id)) errors.push(`duplicate firewall rule id: ${rule.id}`);
    ids.add(rule.id);
  }
  const probe = classifyToolResultContent({ value: "ordinary data" });
  if (probe.trust_class !== UNTRUSTED_TOOL_OUTPUT) errors.push("tool output must be untrusted by default");
  if (probe.instruction_handling !== "data_only") errors.push("tool output must be data-only");
  if (probe.promotion_allowed !== false) errors.push("tool output promotion must be disabled");
  return { ok: errors.length === 0, errors };
}

function validatePromptFirewall() {
  return validatePromptContentPolicy();
}
...
module.exports = {
  ...
  buildModelVisibleContentBoundary,
