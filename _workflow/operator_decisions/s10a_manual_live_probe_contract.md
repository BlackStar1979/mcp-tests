...
`_workflow/scripts/s10_manual_sessionless_probe_stub.js` is a non-network stub with `--self-test`. It records the manual execution boundary without embedding the blocked protected request sequence.
...
`_tests/smoke_s10a_manual_live_probe_contract.js`

## Next recommendation

Run the manual probe from the PC shell outside the ChatGPT tool layer and paste only sanitized JSON output.
