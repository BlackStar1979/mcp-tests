...
`output_dlp_policy` is now implemented, raising current coverage to `14/24`; ten required rows remain outside implemented: four partial, four specified-only, and two critical (`consent`, `prompt/content`). The next product package is `POL-1A-CONSENT`; the authenticated-connector refresh checkpoint is now closed by operator UI evidence corroborated in the server audit.
...
A later operator reconnect attempt exposed a bounded regression from the transport-security hardening: the global Origin gate intercepted POST `/oauth/operator-login` and returned `403 Invalid Origin`. A RED regression in `_tests/smoke_server_factory_extraction.js` proved the authorization-server route never reached dispatch while `/mcp` still needed invalid-Origin rejection. Source commit `2ae1585` scopes mandatory Origin validation to `/mcp`; full pre-deploy validation remained GREEN at `7 + 308`. Controlled restart `manual-1787064834593` loaded that source at `server_start_id = 2026-08-18T14:53:56.306Z`. Live probes prove `/oauth/operator-login` with `Origin: null` now reaches the OAuth handler while `/mcp` with that Origin still returns `403 Invalid Origin`, and durable process recovery remains green. Separately, the operator's earlier green refresh is corroborated by audit: `openai-mcp 1.0.0` completed `server/discover` and `tools/list` with HTTP `200`; `tools_list_served` recorded `98` tools and fingerprint `93721a82a339f9d6` with `ttl_ms = 0` and `cache_scope = private`. Therefore `restart_required_now = false` and `connector_refresh_required_now = false`; normal progression resumes at `POL-1A-CONSENT`. Final post-live full-suite job `bed1f1a4-0418-47db-99d6-1aa45d201041` is GREEN at `7 public + 308 authenticated`, exit `0`, empty stderr, and no truncation.