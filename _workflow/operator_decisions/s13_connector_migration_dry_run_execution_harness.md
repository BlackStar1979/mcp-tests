...
Added and executed `node _workflow/scripts/s13_connector_migration_dry_run_harness.js`.
...
- `/mcp/sessionless` remains candidate future connector target;
...
The active connector route remains `/mcp`. The candidate future route remains `/mcp/sessionless`. The route switch itself was not performed.
...
Alternative S14: stable `/mcp` and `/mcp/sessionless` coexistence regression hardening.
