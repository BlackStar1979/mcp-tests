...
- Policy coverage remains `POL-1 = 2/4` with `15/24` required policies implemented. Current protocol/SEP authority is `SERVER_PROTOCOL_CAPABILITY_SPEC.json`, backed by `src/runtime/protocol_capability_registry.js`; `consent_policy` remains implemented, but its capability-adaptive authorization semantics are repository-GREEN and pending controlled live acceptance, while `prompt_content_policy` remains the critical gap.
- Standards cleanup and production MRTR remain live. OAuth21/internal currently runs the temporary recovery runtime at `server_start_id = 2026-08-20T16:36:54.796Z`; `tools/list` remains `98` with fingerprint `93721a82a339f9d6`. Capability-adaptive `POL-1A-CONSENT` semantics are repository-implemented but not yet live-loaded, so `restart_required_now = true`; `connector_refresh_required_now = false` because tool names and descriptor/input/output schemas are unchanged.
...
1. Complete controlled live acceptance for the capability-adaptive `POL-1A-CONSENT` authorization semantics; then execute `POL-1A-PROMPT` to close the untrusted-content/instruction-promotion boundary.
2. Follow with `POL-1B` and `POL-1C`; keep `COMP-1A` event-gated until newer real Codex entry traffic exists.
3. Preserve production MRTR, `OPS-1`, live Tasks/Trace/Artifacts/CIMD, completed `DOC-2`, and the modular protocol-registry boundary as regression lines; preserve the historical process-MRTR acceptance as MRTR evidence while completing the superseding capability-adaptive `POL-1A-CONSENT` activation.
