...
- Project-local CBM operating guidance is installed at `.agents/skills/using-codebase-memory/SKILL.md` and guarded by `_tests/smoke_cbm_agent_skill.js`; it now tells agents to prefer structured `bridge_analysis`, `impact_resolution_reason`, `runtime_edge_creation`, semantic-only suppression metadata, discovery absence, `source_bearing_excluded_dirs`, Windows non-ASCII, whitespace path/project, and Cypher caveat metadata over warning prose. Upstream issue reviews are recorded through ordinal 200 in `docs/CBM_UPSTREAM_ISSUES_FIRST50_REVIEW.md`, `docs/CBM_UPSTREAM_ISSUES_51_100_REVIEW.md`, `docs/CBM_UPSTREAM_ISSUES_101_150_REVIEW.md`, and `docs/CBM_UPSTREAM_ISSUES_151_200_REVIEW.md`.
- Generator-owned directory orientation now covers `src/integrations`, `src/integrations/codebase_memory`, `_workflow/operator_decisions`, `.agents/skills/using-codebase-memory`, and `.agents/skills/using-codebase-memory/references`. `scripts/audit_directory_docs.js` ranks tracked high-churn directories by `DIRECTORY.md` coverage and currently reports no missing maps among the top 20 dirs with churn >= 7 in the last 30 days; this bounded `DOC-2A` fallback changes no runtime or connector contract.
