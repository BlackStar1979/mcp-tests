...
- The final MCP `2026-07-28` dual-era adapter and DCR `application_type` compatibility policy are live at `server_start_id = 2026-08-01T20:29:11.207Z`. Fresh `COMP-1A` evidence selects `server_start_id = 2026-08-01T17:51:19.986Z`, where `codex-mcp-client 0.146.0-alpha.9.2` on protocol `2025-06-18` remains `initialize_only`, with `2` legacy entries and `0` `server/discover` entries. Server readiness is no longer the blocker; the compatibility shim remains required by operational client behavior.
...
Next recommended action: keep `COMP-1A` event-gated until traffic newer than the August 1 `codex-mcp-client 0.146.0-alpha.9.2` entry window creates a meaningfully new entry-path sample. Preserve the live dual-era adapter and run `DOC-2A` only when a current orientation audit exposes a real gap.
