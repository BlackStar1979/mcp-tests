...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=279`.
- Latest validated public section count: `7`.
- Latest validated authenticated smoke count: `279`.
- Repo current connector-visible authenticated tool target is `91` (`13 public + 78 authorized-visible`), including six async process lifecycle tools; `26` server-internal helpers remain intentionally hidden from MCP schema/tools-list.
- `PROC-1B` is accepted and live on the `91`-tool runtime at `server_start_id = 2026-08-09T18:09:11.902Z`: SQLite WAL persists owner-scoped job metadata, bounded output, and append-only transitions; completed jobs survive restart; orphans become `interrupted`; commands are never replayed; args/env are not persisted; process domain failures return controlled envelopes; Windows `py`, `powershell`, and `pwsh` resolver regressions are covered. Follow-up `PROC-1B-R1` adds renewable server-instance leases and periodic reconciliation after independent stress proved that PID existence alone could preserve a crashed job indefinitely after PID reuse. Two `25/25` persistence runs, a `100/100` hermetic operational soak, and full `7 + 279` validation are green. Controlled restarts `manual-1786298815144` and `manual-1786298950491` loaded the correction; live job `6faee6e9-97a5-4013-bc8e-e8e8ab081d7a` retained `ok`, output `PROC1B_R1_LIVE_OK`, event history, and `recovered_after_restart=true`; SQLite reports exactly one current lease; OAuth remained callable and no lease-maintenance failure was audited.
- `SURF-1` is restored to `4/4`: the refreshed Codex environment exposes all `91` names and directly called `process_list` plus `process_events`; the latter returned the durable `queued -> running -> ok` transition history for connector-owned job `b247a65e-7cea-4e83-9bb8-4cdf66ffb227`. No OAuth relogin or server restart was required.
...
- `RETR-1-QUALITY-REGRESSION` is complete. The live v3 knowledge index contains `262` documents, visited `1131` files and `50` directories, reports freshness `fresh`, extracts all `12` readiness rows, and links all `36` source-of-truth documents. Persisted samples remain bounded; complete canonical extraction is transient and bounded at `512 KiB`.
