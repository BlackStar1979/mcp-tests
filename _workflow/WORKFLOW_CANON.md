...
Next recommended action: keep the live-accepted protocol and completed DOC-2 regression lines stable. `COMP-1A` reopens only on Codex client-entry traffic newer than the August 17 `0.148.0-alpha.9` sample; `OPS-1B` waits only for a real live SFTP boundary.
