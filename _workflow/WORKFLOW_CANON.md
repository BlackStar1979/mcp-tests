...
- `OBS-1-LIVE-TRAIL` is accepted at `4/4`: the current post-restart window was correctly classified as stale entry evidence despite `11` successful follow-up tool calls, retained windows consistently identified the operational Codex initialize blocker, and paired response evidence remained available without raw payload logging. All named-option local JavaScript CLI entrypoints now converge on shared fail-closed parsing with explicit repeatable options where required; rejected arguments cannot silently alter scope, select implicit defaults, or reach high-impact mutation behavior.
