...
- Live OAuth21 `3008` and the connector enumerate tool count `84`, with current live proof `server_start_id = 2026-07-29T18:21:15.570Z`; the connector-visible hardened CBM v0.9.0 contract, including `state_handle`, partial-result metadata, semantic-only suppression, source-bearing exclusion warnings, Windows path/project caveats, Cypher shape caveats, and snippet source-integrity validation, is live and verified without connector-visible surface change.
...
- Fresh `COMP-1A` evidence separates current follow-up traffic from the newest real entry window. The current live `server_start_id = 2026-07-29T18:21:15.570Z` has no newer external entry evidence; `--latest-entry-window` selects `server_start_id = 2026-07-28T03:49:00.666Z`, where operational `codex-mcp-client 0.146.0-alpha.3.1` on protocol `2025-06-18` remains `initialize_only`, with `2` successful legacy responses and `0` `server/discover` entries. Test child-server audit paths are hermetic, historical synthetic starts remain preserved, and the compatibility shim remains required.
...
- Workflow progress markers: `current_working_course = initialize-retirement-evidence-wait`; `next_primary = comp-1a-on-fresh-external-client-traffic`; `next_secondary = bounded-doc-orientation-maintenance`.
...
1. Keep `COMP-1A` event-gated until new external client traffic creates a meaningfully new evidence window.
...
Next recommended action: preserve the hardened OAuth21 `3008` and unchanged 84-tool surface. Do not rerun `COMP-1A` until traffic newer than the July 28 `codex-mcp-client 0.146.0-alpha.3.1` entry window creates a meaningfully new entry-path sample. A single bounded `DOC-2A` package is the only autonomous fallback when a current orientation gap is demonstrable. No public `3009` start is required.
