...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=310`.
...
- Live policy coverage remains active as `POL-1 = 2/4` with `14/24` required policies implemented. Current protocol/SEP authority is `SERVER_PROTOCOL_CAPABILITY_SPEC.json`, backed by `src/runtime/protocol_capability_registry.js`; consent is repository-implemented but remains outside live coverage until controlled activation is accepted.
- Standards cleanup and production MRTR remain live on the existing runtime; `tools/list` remains `98` with fingerprint `93721a82a339f9d6`. The repository now contains the complete selected-process consent boundary, so `restart_required_now = true`; `connector_refresh_required_now = false` because tool names and descriptor/input/output schemas are unchanged.
- MRTR (`SEP-2322`) is production-loaded and live accepted as the existing protocol primitive. `POL-1A-CONSENT` now classifies `run_process`, `process_start`, and `process_cancel` into exact-call-bound MRTR human approval, verifies only the exact accepted `human_approval` response, emits safe receipts, and gates Tasks/jobs/execution before side effects. The consent activation itself is repository/pre-deploy truth only until the supervisor restart and live semantic probes complete.
- Workflow progress markers: `current_working_course = protocol-capability-module-convergence`; `next_primary = pol-1a-consent-boundary`; `next_secondary = pol-1a-prompt-content`.
...
1. `POL-1A-CONSENT`: repository implementation is complete; finish full pre-deploy validation, controlled restart, and semantic live acceptance without accepting model-supplied confirmation as human evidence.
2. `POL-1A-PROMPT`: close the untrusted-content/instruction-promotion boundary.
3. Follow with `POL-1B` and `POL-1C`; keep `COMP-1A` event-gated until newer real Codex entry traffic exists.
4. Preserve accepted production MRTR, `OPS-1`, live Tasks/Trace/Artifacts/CIMD, completed `DOC-2`, and the modular protocol-registry boundary as regression lines.

Next recommended action: complete `POL-1A-CONSENT` pre-deploy/full-suite validation, then perform the controlled supervisor restart and live semantic acceptance on the production MRTR boundary.
