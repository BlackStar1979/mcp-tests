...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=294`.
...
- Workflow progress markers: `current_working_course = trace-context-and-process-artifacts`; `next_primary = trace-context`; `next_secondary = process-artifacts`.
...
1. Execute `TRACE-CONTEXT` as the active internally actionable protocol package: validate `traceparent`, persist only bounded correlation identifiers, and keep trace metadata out of authorization.
2. Follow with `PROCESS-ARTIFACTS`: owner-bound immutable output artifacts with opaque links, hashes, retention, and bounded reads tied to the correlation spine.
...
Next recommended action: execute the bounded `TRACE-CONTEXT` package on repository truth, preserving the current live OAuth21 `3008` runtime until a separate controlled load is justified. `PROCESS-ARTIFACTS` follows after the trace schema is stable. `COMP-1A`, `OPS-1B`, and `DOC-2A` remain event-gated or bounded fallback work.
