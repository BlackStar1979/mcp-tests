...
- `MCP-TASKS-PROCESS-ADAPTER` is repo-validated and closed at the repository layer. Modern clients opting into `io.modelcontextprotocol/tasks` may receive a Task from `run_process`; the Task is a thin owner-bound view over the same durable process registry, `process_start` remains the custom async API, `tasks/list` is absent, terminal results are reconstructed from bounded durable output, and SQLite/WAL reopen preserves completed Tasks. Clean-history validation run `31784712342` passed `7 + 294` plus clean-tree checks. This package is not live-loaded on OAuth21 `3008`; no restart, connector refresh, or OAuth relogin occurred.
- `TRACE-CONTEXT` is repo-validated and closed at the repository layer. Modern `_meta.traceparent` creates a bounded request span; process execution receives a child span; only trace/span ancestry, flags, and source persist in owner-bound SQLite/WAL; invalid trace context restarts locally without failing the MCP request; and raw baggage/tracestate never enter durable state or audit. Full run `31801918351` passed `7 + 298` plus clean-tree checks. This package is not live-loaded on OAuth21 `3008`; the connector-visible 98-tool surface remains unchanged.
- `PROCESS-ARTIFACTS` is repo-validated and closed at the repository layer. Terminal stdout/stderr is materialized immutably in the same owner-bound SQLite/WAL database with opaque IDs, SHA-256, independent bounded retention, W3C correlation metadata, private bounded `resources/read`, no `resources/list` enumeration, and standard `resource_link` content for large Task-backed results. Final squashed source commit `8d8a70b029aa3294d72edc20e9d00c27e2fe8527` is based directly on `TRACE-CONTEXT` commit `786ab533ea7345f82e127715381ee46b72eceeca`; clean-history validation run `31957637766` passed `7 + 301` plus targeted source-tree and clean-tree checks. This package is not live-loaded on OAuth21 `3008`; the connector-visible 98-tool surface remains unchanged.
- `CIMD` is repo-validated and closed at the repository layer. HTTPS Client ID Metadata Document identifiers resolve as ephemeral OAuth clients under `draft-ietf-oauth-client-id-metadata-document-02`: DNS/IP validation covers special-use ranges, transport pins the selected address, automatic redirects are rejected, response processing is bounded to 5 KiB, private JWK material is rejected, HTTP freshness including `Age` is honored, `no-store` and failures are not cached, and CIMD redirect URIs use exact textual matching without changing DCR loopback semantics. Final source commit `c6b706b4083dc4da67a45cc31356c2ffe4edb175` is based directly on P2 truth-sync `f1329f3ac5c0be3c098888967a7541eb7249e312`; clean-history validation run `31959846622` passed `7 + 304` plus targeted OAuth/CIMD and clean-tree checks. This package is not live-loaded on OAuth21 `3008`; the connector-visible 98-tool surface remains unchanged.
- `MRTR` is repo-validated and closed at the repository layer as fixture-only conformance evidence. The official v2 client proves `input_required` elicitation, one retry with a fresh JSON-RPC id, preserved tool arguments, byte-exact opaque `requestState`, keyed `inputResponses`, and final `complete` semantics without modifying production `src/` code or creating another task/session store. Functional commit `a80f9e63b064d22717b5bdfa0bcb4f50c0811414` passed clean-history validation run `31963602072` at `7 + 305` plus pre/post clean-tree checks. This package is not live-loaded on OAuth21 `3008`; the connector-visible 98-tool surface remains unchanged. Next compatibility evidence gate: client traffic newer than the August 14 Codex sample.
...
- Workflow progress markers: `current_working_course = event-gated-compatibility-and-operational-quality`; `next_primary = comp-1a-on-fresh-external-client-traffic`; `next_secondary = ops-1b-on-live-boundary`.
...
1. Preserve repo-complete `MRTR`, `CIMD`, Tasks, Trace Context, and Process Artifacts as regression evidence; their live loading remains a separate operational boundary.
2. Keep `COMP-1A` event-gated until operational Codex client-entry traffic newer than the August 14 `0.147.0-alpha.6.6` sample exists; do not reopen initialize retirement while that family remains `initialize_only`.
3. Reopen `OPS-1B` only when live SFTP infrastructure, a real reconnect event, or a reproduced operational defect creates decision-relevant evidence.
...
Next recommended action: preserve the `7 + 305` MRTR/CIMD/Tasks/Trace/Artifacts regression line and wait for decision-relevant external evidence. `COMP-1A` reopens only on Codex client-entry traffic newer than the August 14 `0.147.0-alpha.6.6` sample; `OPS-1B` remains live-boundary gated, and `DOC-2A` remains closed while the top25/churn>=5 audit reports no missing map.
