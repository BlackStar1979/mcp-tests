...
- Every new work item must declare: `server_change`, `workflow_change`, `schema_change`, `runtime_restart_required`, `connector_refresh_required`, `backup_required`, `rollback_path`, `restore_path`.
...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=313`.
- Latest validated public section count: `7`.
- Latest validated authenticated smoke count: `313`.
- Repo current connector-visible authenticated tool target is `98` (`13 public + 85 authorized-visible`), including six async process lifecycle tools and seven structured-file tools; `26` server-internal helpers remain intentionally hidden from MCP schema/tools-list.
- `PROC-1B` is accepted and remains live on the `98`-tool runtime at `server_start_id = 2026-08-13T18:45:01.480Z`: SQLite WAL persists owner-scoped job metadata, bounded output, and append-only transitions; completed jobs survive restart; orphans become `interrupted`; commands are never replayed; args/env are not persisted; process domain failures return controlled envelopes; Windows resolver regressions are covered; absolute `cwd` values are accepted only inside configured workspace roots. The synchronous runner advertises a conservative 90-second maximum below Cloudflare's documented 125-second Proxy Read Timeout, while `process_start` retains 600 seconds. Full `7 + 280` validation ran through `process_start`; job `ae1e132c-3d73-4258-8d69-5c0c1d43f491` survived the controlled restart with `recovered_after_restart=true`, and post-restart job `52d99026-c73b-4dbd-b312-121125a5e0f6` returned `POST_RESTART_ASYNC_OK`; OAuth remained callable.
- `PROC-1B-R2` is accepted at `server_start_id = 2026-08-13T19:41:25.994Z`, fingerprint `ec7d3af5b4ea17f5`: a durable owner/key reservation and `queued` job are committed atomically before spawn; equal effective retries return one execution across queue exhaustion and restart; conflicting reuse fails closed; OAuth runtime keys use keyed HMAC; raw keys, args, and env remain absent from durable state and audit. Full `7 + 291` passed and live job `1121654d-5016-4cac-84e3-30b4d69630de` was reused after restart. That requirement is satisfied by the repo-validated `MCP-TASKS-PROCESS-ADAPTER`; no second task store was created.
- `MCP-TASKS-PROCESS-ADAPTER` is repo-validated, live-loaded, and live-accepted. Modern clients opting into `io.modelcontextprotocol/tasks` receive a thin owner-bound Task view over the existing durable process registry; non-Tasks clients retain synchronous `run_process`. Clean-history validation is `7 + 294`. Controlled restart `manual-1786907075795` loaded final source `737cdc8b97ec1e966823dc2566eb7d5cd221e9b6` at `server_start_id = 2026-08-16T19:04:37.288Z`; a live Tasks client proved advertised capability, Task creation, `tasks/get` completion, stdout recovery, argv redaction, and task-backed metadata.
- `TRACE-CONTEXT` is repo-validated, live-loaded, and live-accepted. Clean-history validation is `7 + 298`. Live `run_process` proved request → execution child-span ancestry, and the Task-backed live execution persisted the same W3C trace/span/parent metadata through job and artifact state without raw baggage/tracestate persistence.
- `PROCESS-ARTIFACTS` is repo-validated, live-loaded, and live-accepted. Clean-history validation is `7 + 301`. The live Task execution materialized immutable stdout/stderr artifacts in the owner-bound SQLite/WAL store with opaque IDs, SHA-256 and W3C ancestry; authenticated `resources/read` returned the expected stdout via an explicit `mcp-artifact://process/<opaque-id>` URI while `resources/list` remains non-enumerating.
- `CIMD` is repo-validated, live-loaded, and live-accepted. Clean-history validation is `7 + 304`. A live `/authorize` probe using `client_id = https://127.0.0.1/oauth/client.json` was rejected as `invalid_client`, and audit recorded `oauth21_cimd_rejected` with `reason = cimd_special_use_ip`, directly proving the SSRF boundary on the loaded runtime.
- `MRTR` is production-loaded and live accepted at `7 + 310`. Source `476038d` is live at `server_start_id = 2026-08-18T19:03:44.754Z`; `src/runtime/mrtr_extension.js` reuses the bounded state-handle store, persists digest-only call bindings, and gates execution before `tool_call_start`. The module remains dormant until policy emits `mrtr_requirement`; the runtime still exposes `98` tools with fingerprint `93721a82a339f9d6` and needs no connector refresh.
- `SURF-1` remains `4/4`: the refreshed Codex environment exposes all `98` names and directly called the seven structured-file tools plus the accepted process lifecycle surface. No OAuth relogin was required.
- `FILE-1` is accepted at `4/4`: restart `manual-1786646699884` loaded fingerprint `05a85f87dfb3304d`. Live connector preview/commit calls proved owner-bound staging, staged append, hash-bound Markdown section replacement, source-preserving three-part split, and a byte-identical merge. Runtime policy classifies these reversible/source-preserving mutations as non-destructive while retaining OAuth, internal-profile, workspace-root, hash, receipt, backup, and journal controls.
...
- CBM bridge sample-repo stress is now a manual regression tool: `_tests/stress_cbm_bridge_samples.js` repeatedly exercises all exposed CBM bridge tools against selected `_repos_with_code_samples`. The latest strict default run recorded `663` exact calls across all fifteen `cbm_*` tools, including lifecycle index/delete coverage, with `instability=[]`; earlier large-repo stress also stayed stable. `_tests/smoke_cbm_live_bridge_stress.js` also asserts live `cbm_detect_changes` partial-result metadata and `cbm_ingest_traces` runtime-edge placeholder metadata. Volatile native `search took <ms>` warnings no longer affect stable `cbm_search_code` result signatures.
- Test processes are isolated from operational control state: self-test skips restart-controller startup and tool-surface persistence; every smoke child-server uses `_tests/helpers/hermetic_server_control_env.js`; standalone harnesses, ordinary self-test, and full `run_all` preserve the restored 84-tool `_control/tool-surface-state.json` byte-for-byte. Snapshot creation resolves repository scope from its script location, stages content and manifest under a unique `.pending-*` directory, retries only transient Windows `EACCES`/`EBUSY`/`EPERM` failures while preserving same-volume rename publication, and rolls staging back on terminal failure. `run_all_smokes.js` anchors server, auth patch, sandbox, child-process, and relative-manifest paths to the repository; options are parsed by the shared fail-closed parser, child-server exit is awaited, and per-run OAuth/process/audit storage is removed in an outer `finally`. The full-suite no-pollution guard executes the inner run from a foreign cwd and verifies unchanged audit, snapshot, and run-temp sets. One-time cleanup removed `684` snapshot artifacts while preserving `17` named operational snapshots, then removed `298` stale run-temp stores (`72,496,903` bytes) while conservatively retaining `13` PID-occupied directories. These corrections affect workflow/test processes only and require no live-server restart.
- Port `3008` is reserved for the supervisor-managed default OAuth21 runtime. Any independent local server instance must use a different port and hermetic control-state paths; starting a second process on `3008` can displace the supervisor and is prohibited.
...
- Project-local CBM operating guidance is installed at `.agents/skills/using-codebase-memory/SKILL.md` and guarded by `_tests/smoke_cbm_agent_skill.js`; it now tells agents to prefer structured `bridge_analysis`, `impact_resolution_reason`, `runtime_edge_creation`, semantic-only suppression metadata, discovery absence, `source_bearing_excluded_dirs`, Windows non-ASCII, whitespace path/project, Cypher caveat metadata, and the CBM-vs-knowledge-index boundary over warning prose. Upstream issue reviews are recorded through ordinal 277 in `docs/CBM_UPSTREAM_ISSUES_FIRST50_REVIEW.md`, `docs/CBM_UPSTREAM_ISSUES_51_100_REVIEW.md`, `docs/CBM_UPSTREAM_ISSUES_101_150_REVIEW.md`, `docs/CBM_UPSTREAM_ISSUES_151_200_REVIEW.md`, and `docs/CBM_UPSTREAM_ISSUES_201_277_REVIEW.md`.
- `RETR-1-QUALITY-REGRESSION` remains accepted. The refreshed live v3 knowledge index contains `301` documents, visited `1265` files and `48` directories, reports freshness `fresh`, extracts all `15` readiness rows, and links all `37` source-of-truth documents through `791` internal links. Persisted samples remain bounded; complete canonical extraction is transient and bounded at `512 KiB`.
- `DOC-1` is accepted at `4/4`: a fresh single `index_status` response carries the current package markers, full component maturity/blocker/default-package rows, ordered roadmap actions, canonical health, runtime identity, and documentation gaps. The persisted-status regression and live planning/maturity/proof queries prevent autonomous selection from depending on manual cross-reading.
- `DOC-2` is accepted at `4/4`: all `68/68` directories containing tracked files have functional `DIRECTORY.md` maps, including the active CBM native-contract boundary and explicitly quarantined retired validator snapshot. Generator `--help`/`--check` are non-mutating, invalid arguments fail before writes, dynamic control-plane maps have one generation authority, and repo-wide coverage is regression-guarded.
- The final MCP `2026-07-28` dual-era adapter and DCR `application_type` compatibility policy are live. Official `@modelcontextprotocol/client@2.0.0` interoperability proves default legacy plus automatic and pinned modern negotiation with real list/call traffic and server-side path evidence. A separate hermetic OAuth21 regression proves SDK-driven DCR, PKCE S256, host-side `state` validation, RFC 9207 `iss`, issuer-stamped persistence, authenticated list/call in both eras, process-restart recovery of the registered client and active tokens from SQLite, automatic refresh-token rotation during legacy `initialize` after the restart and a rejected access token, and reuse of the rotated credentials by a fresh modern client. Fresh August 17 `COMP-1A` evidence now shows `codex-mcp-client 0.148.0-alpha.9` remains `initialize_only` in the current entry window, with `3` successful legacy `initialize` entries and `0` `server/discover` entries, while independent operational `openai-mcp 1.0.0` traffic uses `server/discover` with protocol `2026-07-28`. Modern server entry is therefore operationally proven, but the compatibility shim remains required by the Codex client family.
- `DEBT-1-RETIREMENT` is complete and live-loaded: unreachable `McpSession`, classic Sampling context, SSE outbound queuing, and pending response resolution are removed. The surviving `/mcp` route initiates no JSON-RPC requests and rejects unsolicited response envelopes with `server_initiated_requests_not_active`. Controlled restart `manual-1786939596474` loaded repository head `e32fefdb4d0f369a9ff30e1ecba7f94600807130` at `server_start_id = 2026-08-17T04:06:37.912Z`; the `98`-tool surface and governed fingerprints remained unchanged, direct `workbench.get_info` succeeded, and no connector refresh or OAuth reauthorization was required.
- `OBS-1-LIVE-TRAIL` is accepted at `4/4`: the current post-restart window was correctly classified as stale entry evidence despite `11` successful follow-up tool calls, retained windows consistently identified the operational Codex initialize blocker, and paired response evidence remained available without raw payload logging. All named-option local JavaScript CLI entrypoints now converge on shared fail-closed parsing with explicit repeatable options where required; rejected arguments cannot silently alter scope, select implicit defaults, or reach high-impact mutation behavior. Relative paths in `patch_section_by_markers.js` and `compact_runtime_logs.js` are repository-root anchored independently of caller cwd, while explicit absolute paths remain supported.
- Shared root-spec loading and active fixture/decision/contract/index-authority read-only CLIs derive repository scope from their script location rather than caller cwd. Explicit relative spec arguments are repository-root relative, absolute paths remain supported, and aggregate plus dedicated foreign-cwd regressions guard the boundary.
- `public_sandbox_sync.js` is retired from the active control plane. Non-authoritative public-sandbox copies remain valid only as historical or explicitly generated evidence; no unguarded helper may duplicate current runtime source into that ignored tree.
- Native CBM probes and tool invocations must use the resolved authorized workspace root as both subprocess cwd and `CBM_ALLOWED_ROOT`; server-launch cwd is not a repository-selection authority.
- Deploy-decision guard v2 normalizes relative, absolute, and Windows-style paths and treats startup-loaded `src/**`, `tools/**`, `profiles/**`, `plugins/**`, root `SERVER_*_SPEC.json`, and package manifests as restart-requiring runtime changes. Documentation/tests remain repo-only, `src/stage_metadata.js` retains its specialized compatibility-status restart workflow, and descriptor/schema/tool-surface flags remain the only connector-refresh/operator-approval triggers. Restart `manual-1786980050739` loaded commit `f2a574b` at `server_start_id = 2026-08-17T15:20:52.127Z`; live guard v2 reproduced the corrected result without OAuth reauthorization or fingerprint drift.
...
- Policy coverage remains `POL-1 = 3/4`. Repository and live OAuth21 target coverage are aligned at `20/24` implemented after live-accepted `POL-1B`, with only four non-critical `specified_only` rows left for `POL-1C`. All `6/6` critical policies remain live accepted. Current protocol/SEP authority is `SERVER_PROTOCOL_CAPABILITY_SPEC.json`, backed by `src/runtime/protocol_capability_registry.js`.
- Standards cleanup, production MRTR, capability-adaptive `POL-1A-CONSENT`, `POL-1A-PROMPT-CONTENT`, and `POL-1B` remain live. OAuth21/internal currently runs source `6fc195d4ef5e86dceeb89a88fc82c83048d368c4` at `server_start_id = 2026-09-04T15:21:41.190Z`; local health is `ok`, startup maintenance returned `no_candidates`, and the runtime surface remains `98` with fingerprint `93721a82a339f9d6`. `restart_required_now = false`; `connector_refresh_required_now = false` because tool names and descriptor/input/output schemas are unchanged. Post-rebuild Codex callability is not yet reaccepted: CLI auth reports `Not logged in`, so current model/runtime visibility remains false until OAuth reauthentication and a fresh Codex runtime.
- MRTR (`SEP-2322`) remains the existing production protocol primitive for explicit fresh-consent classes. `POL-1A-CONSENT` now uses central `authorization_class` / `consent_mode` semantics: internal OAuth `mcp:tools` is standing authorization for bounded tools; `run_process`, `process_start`, and `process_cancel` default to `consent_mode=none`; `cbm_delete_project` retains `tool_confirmation`; and `mrtr_human_approval` remains available for explicit policy selection with the same exact accepted `human_approval`, replay protection, safe receipts, and missing-capability `-32021` behavior. `destructiveHint` and `tool_policy.destructive` are not authorization authorities. Scope elevation, new-origin egress, and secret/token access remain separate policy classes.
- Workflow progress markers are `current_working_course = protocol-capability-module-convergence`; `next_primary = pol-1c`; `next_secondary = comp-1a`, with `COMP-1A` explicitly event-gated until newer real Codex client-entry evidence exists.
...
- Workflow file count is not a project-progress metric. Active navigation is this canon plus `_workflow/ACTIVE_WORKFLOW_INDEX.md`; retained evidence volume does not define progress.
- Earlier checkpoint-specific implementation boundaries remain historical evidence unless reaffirmed by later records.
- Boundary-control review correction: the workstream boundary review is defined by its control decision, not by a supporting coverage package.
- Changing `src/stage_metadata.js` is a runtime-imported code change; changing any startup-loaded source/spec still requires per-change deploy classification.
- Changing startup-loaded source/spec files requires per-change deploy classification; workflow rules must remain smoke-guarded.
...
1. Execute bounded `POL-1C` reconciliation for `memory_policy`, `database_policy`, `supply_chain_policy`, and `incident_response_policy`; reproduce each current gap before changing implementation or status.
2. Keep `COMP-1A` event-gated until newer real Codex entry traffic exists; it is not executable queue work before that external evidence exists.
3. Preserve production MRTR, live-accepted capability-adaptive `POL-1A-CONSENT`, live-accepted `POL-1A-PROMPT-CONTENT`, `OPS-1`, live Tasks/Trace/Artifacts/CIMD, completed `DOC-2`, and the modular protocol-registry boundary as regression lines; preserve the historical process-MRTR acceptance as generic MRTR evidence.

Next recommended action: execute bounded `POL-1C` reconciliation for the four `specified_only` non-critical policies while preserving live-accepted `POL-1B` as a regression line.
...
Initialize-retirement evidence clarification: the remaining blocker is not missing repo support for no-handshake operation on the surviving route. Repo truth now explicitly shows that `server/discover` works and useful `tools/list` / `tools/call` flow already works without a preceding `initialize`; what remains is client/connector compatibility evidence plus explicit authorization before removing legacy `initialize`.

Initialize-compatibility-shim clarification: bounded dual recognition of legacy `initialize` and modern `server/discover` on the single surviving `/mcp` route is currently authorized only as temporary client compatibility debt. It is not authorization for permanent dual-standard destination architecture, must not reintroduce SSE or route split as target behavior, and must not be used to grow new target-facing protocol semantics on the `initialize` path.
...
Route selection clarification: `_workflow/operator_decisions/single_route_selection_keep_mcp.md` selects `/mcp` as the final surviving route. Hidden `/mcp/sessionless` remains transition-only debt and must not be treated as the destination route.

Debt inventory clarification: `_workflow/operator_decisions/single_route_no_sse_migration_debt_inventory.md` records the confirmed SSE/session/dual-route debt still present in code, specs, and active smoke coverage. It is the required scoping input before any removal patch.

Replacement-package clarification: `_workflow/operator_decisions/keep_mcp_no_sse_replacement_package.md` is the active scoping record for the surviving `/mcp` route. It separates confirmed replacement scope from unresolved no-SSE contract details that must not be guessed.

POST accept cleanup clarification: `_workflow/operator_decisions/keep_mcp_post_accept_json_only_cleanup.md` records the first repo-applied runtime step on the surviving `/mcp` route. Stable POST `/mcp` no longer negotiates SSE; GET SSE teardown remains open work.

GET teardown clarification: `_workflow/operator_decisions/keep_mcp_get_sse_teardown.md` records the second repo-applied runtime step on the surviving `/mcp` route. Stable `GET /mcp` no longer opens SSE and now returns `405`; request-contract migration remains open work.

Request-contract bridge clarification: `_workflow/operator_decisions/keep_mcp_request_contract_bridge.md` records the first additive request-contract migration step on the surviving `/mcp` route. Stable `/mcp` now supports `server/discover` with per-request metadata validation, while legacy `initialize` remains supported as explicit transition debt.

Spec-boundary clarification: current official MCP Streamable HTTP direction still allows request-scoped SSE on POST responses and for `subscriptions/listen`. This repo's no-SSE destination is a stricter project target accepted by the operator; do not misstate it as a generic MCP requirement.
...
Session-bound outbound/sampling scoping clarification: `_workflow/operator_decisions/keep_mcp_session_bound_outbound_sampling_scope.md` is the historical scoping input. `_workflow/operator_decisions/deprecated_sampling_helper_retirement_closeout.md` executes the retirement: `session.js`, `sampling_context.js`, `sendSessionRequest(...)`, and `resolvePendingResponse(...)` are gone, while only fail-closed JSON-RPC response-envelope handling stays contract-relevant for the active surviving `/mcp` route.

Deprecated sampling helper retirement closeout: `_workflow/operator_decisions/deprecated_sampling_helper_retirement_closeout.md` records the source-backed decision, MCP 2026-07-28 deprecation basis, exact runtime removals, canonical spec/event updates, regression boundary, and controlled restart requirement. The package does not introduce a replacement provider integration or connector-visible tool change.

Response-side observability clarification: `_workflow/operator_decisions/keep_mcp_rpc_response_observability_package.md` records the bounded audit-symmetry package on surviving `/mcp`. Active repo code now emits `rpc_response_sent` for standard JSON-RPC success/error paths plus active early-exit rejection paths, but the package intentionally stops at structured summaries rather than raw payload export.
...
Pull-only runtime-package clarification: `_workflow/operator_decisions/keep_mcp_pull_only_tool_surface_freshness_runtime_package.md` records the repo-applied runtime step that removes active `subscriptions/listen` / push behavior and stops advertising `capabilities.tools.listChanged` on stable `/mcp`. Hidden-route retirement remains blocked by the unresolved `state/handle/*` fate.

State-handle fate clarification: `_workflow/operator_decisions/keep_mcp_state_handle_tool_pattern_decision.md` records that prototype-only `state/handle/create`, `state/handle/read`, and `state/handle/destroy` do not survive as route-level methods on the final `/mcp` surface. Explicit state handles remain allowed only as a tool-design pattern carried in ordinary tool results/arguments.

Hidden-route retirement-package clarification: `_workflow/operator_decisions/keep_mcp_hidden_sessionless_route_retirement_package.md` records the repo-applied runtime package that retires `/mcp/sessionless` from active code. `_workflow/operator_decisions/keep_mcp_hidden_sessionless_route_live_verification.md` records the completed controlled OAuth21 `3008` restart and bounded live verification that now make this retirement the current live truth.
...
- `_tests/smoke_control_plane_deploy_rollback.js`: guard that proves prepare, execute, dry-run rollback, and real rollback behavior.
...
Pre-OAuth hardening completed before OAuth implementation. Public `auth:none` surface no longer exposes `code_sample_js`; public tool count is 13. JSON-RPC envelope validation, central tool input validation, access trusted-proxy guard, bearer query-token disabled-by-default, CORS split by auth mode, minimal healthz default, public FS root guard, public bind guard, audit log rotation, MCP request-id null rejection, batch item cap, decision-runtime policy bridge v2, optional Mcp-Session-Id validation, request-id replay rejection for explicit sessions, CORS negative guard, and oversized-batch execution bypass guard are active. Guards: `_tests/smoke_pre_oauth_hardening_guards.js`, `_tests/smoke_mcp_protocol_compliance_guards.js`, `_tests/smoke_session_replay_guards.js`, `_tests/smoke_http_boundary_guards.js`.

Tool input budget guard active: `_tests/smoke_tool_input_budget_guards.js`.

Connector surface split guard active: public SERVER_TOOLS_SPEC surface must equal runtime PUBLIC_TOOL_NAMES; non-public core sampler is restricted core, not public or authorized facade. Guard: `_tests/smoke_connector_surface_split_guard.js`.

Connector refresh readiness spec active: `SERVER_CONNECTOR_SURFACE_SPEC.json` defines public/authenticated/operator connector boundaries. Public connector expected tools must equal runtime `PUBLIC_TOOL_NAMES`; operator mutation tools must not be labeled public. Guard: `_tests/smoke_connector_refresh_readiness_spec.js`. Live connector refresh remains an explicit operator action.

Phase A Streamable HTTP preflight green: `accept_policy.js`, `protocol_version_policy.js`, dispatcher preflight validation, initialize protocol negotiation, and `_tests/smoke_streamable_http_preflight_guards.js` are active. Next implementation phase: Phase B - POST SSE response path.
...
Historical Phase E pending request correlation green at that time: `outbound_request_manager.js`, session pending map, server-originated request id namespace, POST JSON-RPC response acceptance, unknown pending fail-closed, batch response acceptance, and `_tests/smoke_pending_request_correlation.js` were active in the earlier session-bound flow. The remaining response-envelope handling is now bounded local compatibility/runtime support, not an active transport-session migration track.

Historical Phase F sampling readiness green at that time: `sampling_context.js`, context enrichment, optional tool context propagation, capability gate, sampling roundtrip via outbound queue/pending response, `_tests/smoke_sampling_capability_gate.js`, and `_tests/smoke_sampling_roundtrip.js` were active in the earlier session-bound flow. Active surviving-route `/mcp` no longer injects `requestSampling` into runtime context; the helper remains only as bounded compatibility/test coverage.

Phase G OAuth resource-server preflight green: `SERVER_AUTH_SPEC.json`, `auth_oauth.js`, `oauth_metadata.js`, protected resource metadata route, OAuth auth mode, query-token rejection, issuer/audience/scope checks, and `_tests/smoke_oauth_preflight_contract.js` are active. OAuth is implemented as an MCP resource-server validator; local OAuth21 authorization-server mode was later added for TEST MCP on port 3008.

OAuth production hardening plan ready: `_workflow/OAUTH_PRODUCTION_HARDENING_PLAN.md` and `_tests/smoke_oauth_production_hardening_plan.js` are active. Execution order: H1 AS metadata, H2 JWKS/RS256, H3 introspection decision, H4 DCR policy, H5 PKCE policy, H6 key rotation, H7 sampling approval, H8 SSE keepalive/resumability, H9 live connector refresh readiness.

H1 AS metadata integration green: `oauth_authorization_server_metadata.js`, runtime AS metadata provider wiring, Protected Resource Metadata authorization_servers from validated AS metadata, and `_tests/smoke_oauth_as_metadata_integration.js` are active. Next implementation phase: H2 - JWKS/RS256 validation path.

H2 JWKS/RS256 validation green: `oauth_jwks_cache.js`, `oauth_jwt_verify.js`, JWKS/RS256 branch in `auth_oauth.js`, `MCP_TEST_OAUTH_JWKS_FILE` wiring, and `_tests/smoke_oauth_jwks_rs256_validation.js` are active. Next implementation phase: H3 - Introspection path or explicit decision to defer introspection.

H3 OAuth introspection validation green: `oauth_introspection.js`, introspection branch in `auth_oauth.js`, `MCP_TEST_OAUTH_INTROSPECTION_FILE`, and `_tests/smoke_oauth_introspection_validation.js` are active. JWKS/RS256 remains preferred production validation. Next implementation phase: H4 - DCR policy.

H4 DCR policy green: `dynamic_client_registration_policy` in `SERVER_AUTH_SPEC.json` and `_tests/smoke_oauth_dcr_policy.js` are active. DCR mode is supported with manual registration fallback; wildcard redirect URIs are forbidden; MCP server does not implement a runtime registration endpoint. Next implementation phase: H5 - PKCE/client-flow policy at AS boundary.

H5 PKCE client-flow policy green: `pkce_client_flow_policy` in `SERVER_AUTH_SPEC.json` and `_tests/smoke_oauth_pkce_policy.js` are active. Authorization code + PKCE S256, state verification, exact redirect URI validation, and resource parameter in authorization/token requests are policy requirements at AS boundary. Next implementation phase: H6 - Key rotation.

H6 JWKS key rotation green: `oauth_jwks_cache.js` and `_tests/smoke_oauth_key_rotation.js` are active. Next implementation phase: H7 - Sampling user-approval policy.

Historical H7 sampling user-approval policy green at that time: `SERVER_SAMPLING_POLICY_SPEC.json`, runtime approval/budget enforcement in `sampling_context.js`, and `_tests/smoke_sampling_user_approval_policy.js` were implemented for the earlier sampling path. The policy/spec and helper coverage remain preserved, but active surviving-route `/mcp` no longer injects sampling into runtime context.
...
mcp-tests audit remediation green: GET `/mcp` SSE authenticates before opening stream and `/docs/*` malformed percent-encoding returns controlled 400. Guards: `_tests/smoke_get_sse_requires_auth.js`, `_tests/smoke_docs_malformed_uri.js`. Further live connector refresh remains blocked unless explicitly approved by operator.
...
P4 runtime policy expansion scope plan green: `_workflow/operator_decisions/p4_runtime_policy_expansion_scope_plan.md` records the historical planning baseline used before the bounded Stage 14 apply. It must no longer be read as current runtime truth because later records supersede its old "not implemented" state.
...
Stage 7.3 S1 sessionless compatibility inventory green: `_workflow/operator_decisions/stage7_sessionless_inventory.md` and `_workflow/sessionless_inventory.json` map current dependencies on initialize, `MCP-Session-Id`, SessionStore, GET SSE, pending correlation, sampling, replay guard, OAuth21 connector, and explicit non-target batch SSE/DELETE teardown. Conclusion: do not remove session code in Stage 7; next runtime-safe step is request cancellation context plumbing in the current stable-compatible runtime.

Request cancellation context plumbing green: `src/runtime/request_cancellation.js` creates a per-POST AbortSignal from client abort lifecycle and `mcp_entry_dispatcher`, `single_payload_dispatcher`, and `batch_payload_dispatcher` pass `abortSignal` through to `handleRpcMessage` context. Guard: `_tests/smoke_request_cancellation_context.js`. This is historical C1 evidence, not the final cancellation closeout by itself.

Client-disconnect write guard green: `src/runtime/response_write_guard.js` centralizes response-abort checks and single/batch dispatchers plus `rpc_handler_exception_handler` skip response writes after request abort or closed response. Audit event: `response_write_skipped_after_client_disconnect`. Guard: `_tests/smoke_client_disconnect_write_guard.js`. No cooperative tool cancellation, JSON-RPC cancel, sessionless transport migration, connector refresh, or tool surface change was implemented.

Stage 7 closeout green: Stage 7 is closed after repo/live convergence, S1 sessionless compatibility inventory, request abort-signal plumbing, and client-disconnect no-write guard. Stage 7 commits: `20baa5b`, `6271bbc`, `4093ac2`, `d410da7`. Cooperative tool cancellation is intentionally deferred to a separate operator-approved stage. Next approved stage is Stage 8 static registry foundation before hotplug and policy enforcement.

Stage 8.1 static tool registry abstraction green: `src/static_tool_registry.js` adds a read-only static registry over core descriptors and optional tools. `runtime_support_assembly.toolsList()` now renders through this registry while preserving descriptor-equivalent output. Guard: `_tests/smoke_static_tool_registry_equivalence.js`. No dynamic registration, list_changed emission, state-store write, connector refresh, public connector reconnection, or runtime policy enforcement change was implemented.

Stage 8.2 registry-to-policy read model green: `src/registry_policy_read_model.js` adds a read-only model mapping static registry entries to descriptors, runtime `TOOL_POLICIES`, and `SERVER_TOOLS_SPEC.tool_catalog` metadata. Guard: `_tests/smoke_registry_policy_read_model.js`. No runtime enforcement, allow/deny behavior change, connector refresh, public connector reconnection, or hotplug/list_changed emission was implemented.

Stage 8.3 registry diff dry-run green: `src/registry_diff_dry_run.js` adds static registry snapshot diffing with add/remove/update detection and dry-run list_changed harness integration. Guard: `_tests/smoke_registry_diff_dry_run.js`. Existing `tools_list_diff` and list_changed pipeline remain dry-run only. No real registry mutation, tools/list mutation, list_changed emission, connector refresh, public connector reconnection, or hotplug was implemented.
...
Repo hygiene audit green: `_workflow/operator_decisions/post_stage13_repo_hygiene_audit.md` records code-health and workflow-queue hygiene after the earlier closeout milestone. Validation evidence: run_all skip-network returned ok with public=6 and tests_authenticated=157; server self-test returned ok; npm test could not run because npm is unavailable in the process-runner PATH, which is tool-environment ergonomics rather than repo failure. Active remaining work is reduced to six described items: Runtime Enforcement Apply Package - No Apply proposal, Cooperative Tool Cancellation, Event-driven Hotplug Lifecycle, Sessionless / Explicit State Handles Target Selection, Legacy Retired Auth Test Archive/Cleanup, and CRLF Batch Normalization. No implementation approval is recorded; no runtime code patch, deploy, restart, connector refresh, runtime enforcement, allow/deny behavior change, hotplug/list_changed emission, sessionless migration, or mass CRLF normalization was performed.
...
Runtime policy scope-matrix closeout green: `_workflow/operator_decisions/runtime_policy_scope_matrix_closeout.md` closes the bounded D7 ledger item for current repo/live truth. The gate is already wired in `tools/call`, repo specs mark enforcement active, OAuth21 3008 has later live-load evidence, and an optional future live denied-request probe is not required to keep the ledger out of `done`.
Feature lifecycle deprecation-policy closeout green: `_workflow/operator_decisions/feature_lifecycle_deprecation_policy_closeout.md` closes the SEP-2596 governance ledger item for the active inventory layer. Lifecycle/status/migration/checklist structure and smoke coverage already exist in `_workflow/sessionless_inventory.json`, so separately open implementation items remain their own ledger entries rather than blockers for governance itself.
Stage 14.7 D1 observation is closed. D1-A/D1-B/D1-C repo-applied and live-validated on TESTS_MCP 3008.

S9 OAuth21 3008 sessionless activation green: `_workflow/operator_decisions/oauth21_sessionless_activation_trial.md` records hidden `/mcp/sessionless` live activation on 3008 by ignored `_control/sessionless-prototype.json` and supervisor-controlled restart. Runtime status after restart: server_start_id `2026-06-29T09:08:02.097Z`, auth `oauth21`, profile `internal`, 43 tools, tool hash `8b62ecaf89227335`, combined fingerprint `476c7d832021acb9`; `GET /mcp/sessionless` returns 405 and unauthenticated POST returns 401. This is historical activation evidence only: the route has since been retired from active repo runtime and verified absent on live OAuth21 `3008`. No connector refresh, connector migration, public 3009 start, stable `/mcp` removal, or stable session code removal was performed.

S10B live authenticated sessionless probe green: `_workflow/operator_decisions/sessionless_live_authenticated_probe.md` records the repo-native live authenticated `/mcp/sessionless` probe on OAuth21 3008. The new runner `node _workflow/scripts/sessionless_live_authenticated_probe.js` creates fresh OAuth21 client/token material in-process, does not read durable OAuth state, does not log credential material, and keeps connector/runtime unchanged. Passed live checks: health internal/43, `GET /mcp/sessionless -> 405`, unauthenticated POST `401`, authenticated `server/discover` includes `2025-06-18`, `state/handle/create` returns `esh_` handle, owner read succeeds, second client is denied without raw-handle echo, destroy returns true, revoked read does not echo raw handle, and appended audit lines do not contain the raw handle. Connector/runtime remained `43 / 8b62ecaf89227335 / 476c7d832021acb9`. This is historical transition-route evidence only and not current live-route truth. No restart, connector refresh, public 3009 start, connector route migration, stable `/mcp` removal, or stable session code removal was performed.

S11 connector migration readiness / stable `/mcp` coexistence boundary green: `_workflow/operator_decisions/connector_route_coexistence_boundary.md` records the post-S10B no-migration boundary. `/mcp/sessionless` is preserved there only as historical transition-route evidence from OAuth21 3008 probing, while stable `/mcp` remains the legacy-compatible route and must not be removed before separate connector/UI migration evidence exists. No connector route migration, connector refresh, public 3009 start, OAuth21 3008 restart, stable `/mcp` removal, or stable session code removal was performed. Future connector migration remains a separate explicit operator decision; future connector refresh is required only when connector-visible route/surface/auth truth changes or explicit UI validation is requested. The active next recommendation becomes S12 connector migration dry-run plan / no refresh.

S12 connector migration dry-run plan / no refresh green: `_workflow/operator_decisions/connector_migration_dry_run_plan.md` records the first explicit dry-run migration package after S11. It defines a future rehearsal sequence, evidence gates, rollback/restore expectations, and sign-off boundaries without performing connector migration, connector refresh, OAuth21 3008 restart, public 3009 start, stable `/mcp` removal, or stable session code removal. Stable `/mcp` remains preserved while `/mcp/sessionless` remains historical transition-route evidence only. The active next recommendation becomes S13 connector migration dry-run execution harness / still no refresh, with S13 connector refresh approval package / no execution as the alternative.

S13 connector migration dry-run execution harness / still no refresh green: `_workflow/operator_decisions/connector_migration_dry_run_harness.md` records the executed repo-native dry-run harness after S12. The harness validates stable `/mcp` preservation, historical `/mcp/sessionless` transition-route evidence, unchanged connector-visible tool count/hash/auth assumptions, rollback preservation of `/mcp`, and sanitized dry-run output without performing connector migration, connector refresh, OAuth21 3008 restart, public 3009 start, stable `/mcp` removal, stable session code removal, or runtime route behavior change. The active next recommendation becomes S14 connector refresh approval package / no execution, with historical transition-route regression evidence hardening as the alternative.

S14 connector refresh approval package / no execution green: `_workflow/operator_decisions/connector_refresh_approval_package.md` records the first explicit operator/UI package after S13. Because the current Codex UI has no separate refresh button in the confirmed operator environment, the practical refresh-equivalent action is remove + add of the authenticated connector, still targeting stable `/mcp` at `https://mcp-tests-oauth21.romionologic.dev/mcp`. This package is workflow-only: it does not claim that connector removal, connector add, OAuth/password prompt, consent prompt, or visible tool-list validation has already occurred. No connector execution, no OAuth21 3008 restart, no public 3009 start, no connector route migration, no stable `/mcp` removal, no stable session code removal, and no connector-visible tool surface change were performed. The active next recommendation becomes S15 connector reconnect execution evidence on stable `/mcp`, with historical transition-route regression evidence hardening as the alternative.

S15 connector reconnect execution evidence on stable `/mcp` green: `_workflow/operator_decisions/connector_reconnect_execution_evidence.md` records the first actual UI reconnect evidence after S14. The operator removed and re-added the authenticated connector on stable `/mcp`, the UI displayed an authentication prompt, the operator entered the OAuth password, and the connector accepted the password. Claude Code connector UI then confirmed `43` visible tools, grouped as `40` read-only/internal/create and `3` write/delete, aligning with the authenticated 43-tool surface. No connector route migration to `/mcp/sessionless`, no OAuth21 3008 restart, no public 3009 start, no stable `/mcp` removal, and no stable session code removal were performed.

S16 state and snapshot hygiene repair green: `_workflow/operator_decisions/state_and_snapshot_hygiene.md` repairs two workflow-truth defects without runtime change. `_workflow/state.json` was reduced back to an orientation map by removing stage/checklist summaries whose authority belongs in inventory, root specs, or operator records. Control-plane snapshot recursion was repaired by removing nested `_workflow/control_plane/snapshots` subtrees inside snapshots and by adding recurrence guards in `test_mcp_backup.ps1` and `workflow_snapshot.js`. No top-level snapshot directory was deleted wholesale, and no runtime code, connector surface, auth mode, or tool catalog was changed.

Archive-boundary clarification green: `_workflow/historical/README.md`, `_tests/archive/README.md`, and `_tests/archive/legacy_retired_auth/README.md` now make the archive boundary explicit in the repo itself, and `_tests/smoke_state_and_snapshot_hygiene.js` now guards those files. Historical grep hits in those directories remain archival evidence only and must not be treated as current workflow or route authority.

Retired-root-backups clarification green: `_workflow/control_plane/retired_root_backups/README.md` now marks the moved root-backup bucket as archival quarantine, and `_tests/smoke_state_and_snapshot_hygiene.js` now guards that boundary too. Historical grep hits there remain recovery evidence only and must not be treated as current smoke inventory, route truth, or active workflow direction.

S8 isolated sessionless activation regression green: `_workflow/operator_decisions/isolated_sessionless_activation_regression.md` records real local activation on port 3020 with `MCP_TEST_ENABLE_SESSIONLESS_PROTOTYPE=1`, auth `oauth`, profile `internal`, 43 tools, protocol version `2025-06-18`, and route `/mcp/sessionless`. Results passed: post-only 405, auth-required 401, missing protocol header 400, header/meta mismatch 400, unsupported version JSON-RPC `-32004`, `server/discover`, initialize rejection, state handle create/read/deny/destroy/revoked, audit raw-handle absence across 15 audit lines, temp cleanup, and port close. No OAuth21 3008 restart, public 3009 start, connector refresh, connector route migration, stable `/mcp` removal, or stable session code removal was performed.

S7 SEP-2575 sessionless request contract green: `_workflow/operator_decisions/sessionless_sep2575_request_contract.md` records runtime enforcement on hidden `/mcp/sessionless`: `MCP-Protocol-Version` required, `_meta.io.modelcontextprotocol/protocolVersion` required and must match the header, `_meta.io.modelcontextprotocol/clientInfo` and `_meta.io.modelcontextprotocol/clientCapabilities` required, supported sessionless version `2025-06-18`, unsupported version returns JSON-RPC `-32004` with HTTP 400, and `server/discover` returns `supportedVersions`, `capabilities`, and `serverInfo`. Current `/mcp`, connector surface, OAuth21 3008 activation state, public 3009, and stable session code remain unchanged.
...
S6 legacy retired auth cleanup and sessionless-ready spec review green: `_workflow/operator_decisions/legacy_auth_cleanup_sessionless_ready_review.md` records that 17 retired access/bearer smoke files were archived under `_tests/archive/legacy_retired_auth/` with no hard delete, consolidated negative controls now live in `_tests/smoke_legacy_retired_auth_negative_controls.js`, and all 30 root `SERVER*SPEC.json` files carry `sessionless_ready_review`. This is workflow/spec posture only: no runtime code patch, no OAuth21 3008 restart, no public 3009 start, no connector refresh, no connector route migration, and no stable `/mcp` removal.

S5 workbench sessionless standardization green: `_workflow/operator_decisions/workbench_sessionless_standardization.md` records soft deprecation of the stable session-based `/mcp` method for new workbench development only, while preserving `/mcp` as legacy-compatible. `_workflow/sessionless_inventory.json#target_selection_readiness.s5_workbench_deprecation_standardization` is authoritative. At that historical point, the workbench standard was S4 `/mcp/sessionless` with `MCP_TEST_ENABLE_SESSIONLESS_PROTOTYPE=1` for isolated higher-port tests. This is not current destination truth and is superseded by the current single-route target contract. OAuth21 3008 activation and connector migration remained separate explicit operator stages. No runtime code change, restart, public 3009 start, or connector refresh was performed.

S4 sessionless runtime prototype live-loaded: `_workflow/operator_decisions/sessionless_runtime_prototype.md` records hidden `/mcp/sessionless` route gated by `MCP_TEST_ENABLE_SESSIONLESS_PROTOTYPE`, default disabled. Added `src/runtime/state_handle_prototype.js` and `src/runtime/sessionless_prototype_route_handler.js`; OAuth21 auth result now includes `clientId` for state-handle binding. Current `/mcp`, stable session code, connector-visible tool surface, and connector refresh remain unchanged. OAuth21 3008 was restarted through supervisor; live server_start_id is `2026-06-29T05:38:06.443Z`. The route remains default-disabled without `MCP_TEST_ENABLE_SESSIONLESS_PROTOTYPE`; connector-visible tool surface remains 43 tools with hash `8b62ecaf89227335`; connector refresh is not required.

S3 explicit state handle design rules green: `_workflow/operator_decisions/explicit_state_handle_design_rules.md` records no-runtime preparation for S4. `_workflow/sessionless_inventory.json#target_selection_readiness.s3_explicit_state_handle_design_rules` is the authoritative rules source; guard `_tests/smoke_explicit_state_handle_design_rules.js` validates opaque handles, authorization binding, lifecycle/TTL, audit redaction, error contract, and S4 readiness gate. No runtime state handle store, handle-bearing tools, session code removal, POST-only draft mode, connector refresh, public 3009 start, or OAuth21 3008 restart was performed.

Sessionless target selection prepared, then selected, then truth-consolidated: `_workflow/operator_decisions/sessionless_target_selection_preparation.md` records source refresh and target-selection readiness; `_workflow/operator_decisions/sessionless_target_selection_decision.md` closes the missing S2 operator decision and selects `parallel_draft_sessionless_prototype_while_keeping_stable_connector_current`; `_workflow/operator_decisions/sessionless_inventory_truth_consolidation.md` keeps `_workflow/sessionless_inventory.json#target_selection_readiness` as the single authoritative source. Guard `_tests/smoke_sessionless_target_selection_readiness.js` reads inventory directly. Stable `/mcp` remains preserved while `/mcp/sessionless` continues there only as historical parallel-track evidence, not current destination truth.

Target correction green: `_workflow/operator_decisions/no_sse_single_route_target_correction.md` records the operator clarification from 2026-06-30 that SSE and permanent dual-route coexistence are not the desired destination. They remain current migration debt only. Active planning must target one streamable-HTTP-only route with no SSE, sessionless/stateless behavior, and removal of transport-session assumptions.

`subscriptions/listen` compatibility matrix green: `_workflow/operator_decisions/subscriptions_listen_compatibility_matrix.md` records the design boundary between stable `GET /mcp` SSE, future POST-scoped sessionless `subscriptions/listen`, and existing dry-run `notifications/tools/list_changed` work. It confirms that `subscriptions/listen` belongs to the sessionless track, must not depend on `MCP-Session-Id`, and must not reintroduce `Last-Event-ID` replay semantics. No runtime change, restart, connector refresh, public 3009 start, or stable `/mcp` behavior change was performed.

`subscriptions/listen` isolated validation green: `_workflow/operator_decisions/subscriptions_listen_isolated_validation.md` records the first runtime implementation and isolated higher-port validation on the hidden sessionless route. That implementation currently uses request-scoped SSE and therefore must be treated as transitional migration debt rather than the final target. Stable `/mcp`, OAuth21 `3008`, public `3009`, and the connector target remained unchanged. Guard `_tests/smoke_subscriptions_listen_isolated_validation.js` now preserves that historical evidence while also asserting that hidden `/mcp/sessionless` is retired from the current active runtime.
...
Cooperative tool cancellation green: `_workflow/operator_decisions/c3_cooperative_tool_cancellation.md` records repo-applied and live-loaded cooperative cancellation for optional tool execution on OAuth21 3008 (`server_start_id=2026-06-28T18:29:15.549Z`). Earlier cancellation-context and client-disconnect blockers are removed as already implemented. Restart-authority blocker for OAuth21 3008 is removed as invalid current blocker. Connector refresh is not required because no connector-visible tool surface changed. New code: `src/runtime/cooperative_tool_cancellation.js`, updated `src/runtime/optional_tool_call_handler.js`, event `tool_call_cancelled_cooperative`, guard `_tests/smoke_cooperative_tool_cancellation.js`.
...
Adjacent runtime-contract SEP triage green: `_workflow/operator_decisions/adjacent_runtime_contract_sep_triage.md` classifies all 19 Final SEPs in the `adjacent_runtime_contract` bucket into three workflow-only outcomes: already covered by existing runtime/spec/test evidence, explicit non-target or disabled-by-policy areas, and a bounded partial-coverage watchlist. No runtime change, restart, connector refresh, or public 3009 start was performed. The current watchlist is `SEP-2164`, `SEP-1613`, `SEP-1319`, `SEP-1303`, and `SEP-973`.

Auth/security adjacent SEP triage green: `_workflow/operator_decisions/auth_security_adjacent_sep_triage.md` classifies all 7 Final SEPs in the `auth_security_adjacent` bucket into current coverage, explicit non-target/no-current-dependency, and a bounded partial-coverage watchlist. No runtime change, restart, connector refresh, or public 3009 start was performed. The remaining watchlist is `SEP-991`.
