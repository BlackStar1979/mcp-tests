...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=298`.
- Latest validated public section count: `7`.
- Latest validated authenticated smoke count: `298`.
...
- Workflow progress markers: `current_working_course = process-artifacts-and-cimd`; `next_primary = process-artifacts`; `next_secondary = cimd`.
...
1. Execute `PROCESS-ARTIFACTS` as the active internally actionable package: owner-bound immutable output artifacts with opaque links, hashes, retention, and bounded reads tied to the accepted correlation spine.
...
Next recommended action: execute `PROCESS-ARTIFACTS` on repository truth using the accepted W3C correlation spine, while preserving the current live OAuth21 `3008` runtime until a separate controlled load is justified. `CIMD` follows after the artifact contract is stable; `COMP-1A`, `OPS-1B`, and `DOC-2A` remain event-gated or bounded fallback work.
