...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=269`.
...
- Live OAuth21 `3008` and the connector enumerate tool count `84`, with current live proof `server_start_id = 2026-07-29T18:53:50.174Z`; the connector-visible hardened CBM v0.9.0 contract, including `state_handle`, partial-result metadata, semantic-only suppression, source-bearing exclusion warnings, Windows path/project caveats, Cypher shape caveats, and snippet source-integrity validation, is live and verified without tool-name or schema change.
...
- `MEM-1` is live-loaded and full-suite green: append-only JSONL remains authoritative, optional OVH `bge-m3` embeddings use a non-authoritative hash-only SQLite cache, hybrid ranking preserves keyword fallback, egress is disabled unless provider/egress/token are all configured, and vectors or credentials never enter MCP output. Restart `manual-1785351228729` loaded the package at `server_start_id = 2026-07-29T18:53:50.174Z`; own-connector `get_info` and `memory_search` succeeded, no embedding cache was created, and absent runtime configuration keeps behavior keyword-only until controlled activation and a PL/EN quality probe. Memory tool `openWorldHint` annotations changed without names or schemas, so descriptor-only connector metadata refresh/review remains pending.
...
- Workflow progress markers: `current_working_course = memory-embedding-runtime-activation-gate`; `next_primary = mem-1-live-provider-activation-and-quality-proof`; `next_secondary = oauth21-safe-recurring-prune-redesign`.
