...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=278`.
...
- `PROC-1B` is accepted and live on the `91`-tool runtime at `server_start_id = 2026-08-09T16:35:16.355Z`: SQLite WAL persists owner-scoped job metadata, bounded output, and append-only transitions; completed jobs survive restart; orphans become `interrupted`; commands are never replayed; args/env are not persisted; process domain failures return controlled envelopes; Windows `py`, `powershell`, and `pwsh` resolver regressions are covered. Live job `731a623b-497a-4d2d-a30d-c481b8e87114` retained terminal status and output across restart `manual-1786292134573` with `recovered_after_restart=true`, and remained readable after the final truth-baseline restart `manual-1786293314947`; OAuth remained callable.
...
- Workflow progress markers: `current_working_course = operational-e2e-reliability`; `next_primary = ops-1a-operational-e2e-coverage-matrix`; `next_secondary = comp-1a-client-entry-evidence-on-new-version`.
