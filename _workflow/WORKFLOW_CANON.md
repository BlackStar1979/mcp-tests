...
- Live OAuth21 `3008` and the connector enumerate tool count `84`, with current live proof `server_start_id = 2026-07-29T18:00:39.162Z`; the connector-visible hardened CBM v0.9.0 contract, including `state_handle`, partial-result metadata, semantic-only suppression, source-bearing exclusion warnings, Windows path/project caveats, Cypher shape caveats, and snippet source-integrity validation, is live and verified without connector-visible surface change.
...
- Fresh `COMP-1A` evidence separates current follow-up traffic from the newest real entry window. The current live `server_start_id = 2026-07-29T18:00:39.162Z` has no newer external entry evidence; `--latest-entry-window` selects `server_start_id = 2026-07-28T03:49:00.666Z`, where operational `codex-mcp-client 0.146.0-alpha.3.1` on protocol `2025-06-18` remains `initialize_only`, with `2` successful legacy responses and `0` `server/discover` entries. Test child-server audit paths are hermetic, historical synthetic starts remain preserved, and the compatibility shim remains required.
