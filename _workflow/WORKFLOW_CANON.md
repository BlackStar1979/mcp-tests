...
- `PROC-1B` is accepted and live on the `91`-tool runtime at `server_start_id = 2026-08-09T16:15:36.332Z`: SQLite WAL persists owner-scoped job metadata, bounded output, and append-only transitions; completed jobs survive restart; orphans become `interrupted`; commands are never replayed; args/env are not persisted; process domain failures return controlled envelopes; Windows `py`, `powershell`, and `pwsh` resolver regressions are covered. Live job `731a623b-497a-4d2d-a30d-c481b8e87114` retained terminal status and output across restart `manual-1786292134573` with `recovered_after_restart=true`; OAuth remained callable.
