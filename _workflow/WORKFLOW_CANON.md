...
- Policy coverage remains active as `POL-1 = 2/4`. Current repository truth proves `14/24` required policies implemented: transport security rejects invalid present `Origin`; OAuth21/internal tool dispatch enforces granted `mcp:tools`; session security matches the stateless MCP `2026-07-28` target; and output DLP now enforces JSON Schema 2020-12 validation, secret redaction, untrusted-output metadata, resource-link filtering, and embedded-resource handling across optional tools, core search/fetch, and process Tasks. `10` required policies remain outside `implemented`: `4` partial, `4` specified-only, and `2` critical (`consent`, `prompt/content`). Live OAuth21 `3008` still runs source commit `cbbb284` at `server_start_id = 2026-08-17T19:00:17.718Z`; the DLP runtime changes are repo-applied but not live-loaded yet, therefore `restart_required_now = true`. Live connector/runtime truth remains `98` tools / `ec7d3af5b4ea17f5`, but DLP enforcement exposed and repaired ten impossible placeholder output schemas. Repository target fingerprints are now output `8292895f0216967c`, descriptor `82ffaaa3adb6e7db`, combined `93721a82a339f9d6`; names and input schemas are unchanged. The schema/descriptor delta requires connector refresh after the runtime package is loaded.
