...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=277`.
- Latest validated public section count: `7`.
- Latest validated authenticated smoke count: `277`.
- Repo current connector-visible authenticated tool target is `89` (`13 public + 76 authorized-visible`), including four new async process lifecycle tools; `26` server-internal helpers remain intentionally hidden from MCP schema/tools-list.
- `PROC-1A` is accepted and live. Restart `manual-1786285267764` loaded the final `89`-tool OAuth21 runtime at `server_start_id = 2026-08-09T14:21:09.406Z`; official-client OAuth E2E, async lifecycle/audit checks, full `7 + 277` suite, direct live Node/npm/npx/pip/Docker calls, and the source-scoped truth audit are green. This already-open Codex task retains its startup `85`-tool map until re-enumeration, but `workbench` remains authorized and no OAuth relogin is required.
