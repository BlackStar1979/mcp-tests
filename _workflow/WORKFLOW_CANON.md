...
- `OBS-1-LIVE-TRAIL` is accepted at `4/4`: the current post-restart window was correctly classified as stale entry evidence despite `11` successful follow-up tool calls, retained windows consistently identified the operational Codex initialize blocker, and paired response evidence remained available without raw payload logging. The follow-up CLI-integrity sweep centralizes fail-closed parsing across workflow probes, the operational soak runner, and the restart-request helper; rejected arguments cannot silently alter scope or write a default restart trigger.
