...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok_0_40_0_7_221`.
- Latest validated public section count: `7`.
- Latest validated authenticated smoke count: `221`.
- Repo current authenticated tool target after the final modular parity batch: `95`.
- Live OAuth21 `3008` is still at authenticated `/mcp` tool count `82`; the final modular parity batch is repo-applied but not yet live-loaded because no new controlled restart has been executed for this batch. Current live proof remains server_start_id `2026-07-09T23:04:07.382Z`.
- Codex UI connector-side visible-tool enumeration was not independently re-verified in this turn, so current live `82` proof is runtime/MCP truth only, not a fresh UI inventory proof.
- Workflow progress markers: `current_working_course = stage8_53d-live-restart-and-connector-surface-reconciliation`; `next_primary = post_53d-initialize-compatibility-debug-and-retirement-scope`; `next_secondary = post_53d-connector-visible-surface-revalidation`.
...
Next recommended action: use the recovered supervisor-managed restart authority to live-load the repo `95`-tool authenticated surface on OAuth21 `3008`, then capture fresh runtime/MCP evidence before reopening the `initialize` retirement/debug track. Modular parity against `C:\Work\mcp` is now repo-complete, so do not reopen `/mcp/sessionless` target planning as a fallback. Repo-side request flow is already stronger than the old blocker wording implied: the surviving `/mcp` route already supports `server/discover`, and active `tools/list` / `tools/call` already work without a preceding `initialize`. Real client evidence is now stronger too: a freshly re-added Codex desktop Streamable HTTP connector can complete OAuth on a fresh port and still call legacy `initialize` instead of `server/discover`. The first bounded residual cleanup is already repo-applied: unreachable GET-SSE, SessionStore, replay-helper, and push-emitter files were removed from the active repo while stable `/mcp` remained unchanged. The inactive sampling-context injection is now detached from active `/mcp` request handling, the remaining local helpers are explicitly classified as compatibility fixtures rather than active surviving-route wiring, and deprecated roots/sampling/logging inventory no longer remains unknown. The hidden-route retirement package is already repo-applied and live-verified. Connector refresh remains unnecessary because surviving `/mcp` is unchanged. Public 3009 start remains unnecessary unless a narrower record says otherwise.
