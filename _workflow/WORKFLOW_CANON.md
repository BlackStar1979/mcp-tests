...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=304`.
...
- The final MCP `2026-07-28` dual-era adapter and DCR `application_type` compatibility policy are live at `server_start_id = 2026-08-01T20:29:11.207Z`. Official `@modelcontextprotocol/client@2.0.0` interoperability proves default legacy plus automatic and pinned modern negotiation with real list/call traffic and server-side path evidence. A separate hermetic OAuth21 regression now proves SDK-driven DCR, PKCE S256, host-side `state` validation, RFC 9207 `iss`, issuer-stamped credential persistence, authenticated 84-tool list/call in both eras, process-restart recovery of the registered client and active tokens from SQLite, automatic refresh-token rotation during legacy `initialize` after the restart and a rejected access token, and reuse of the rotated credentials by a fresh modern client. Fresh August 2 `COMP-1A` evidence on the same server start still shows `codex-mcp-client 0.146.0-alpha.9.2` as `initialize_only`, with `2` matching legacy entries and `0` `server/discover` entries. Server readiness is no longer the blocker; the compatibility shim remains required by operational client behavior.
...
- Workflow progress markers: `current_working_course = mrtr-and-protocol-compatibility-evidence`; `next_primary = mrtr`; `next_secondary = comp-1a-client-entry-evidence-on-new-version`.
...
1. Execute fixture-scoped `MRTR` as the active internally actionable package: return `InputRequiredResult`, accept matching `inputResponses`, preserve opaque `requestState`, and complete on retry without creating a second execution or session architecture.
2. Preserve repo-complete `CIMD` as regression evidence and treat live OAuth21 `3008` loading as a separate operational boundary.
3. Preserve accepted `MEM-1`, `PROC-1`, `SURF-1`, and completed `OPS-1A` behavior as regression-protected infrastructure.
4. Inspect current traffic for a `COMP-1A` entry window newer than the August 2 operational sample before changing the legacy-initialize verdict; reopen `OPS-1B` only when its external boundary becomes available.
5. Use `DOC-2A` only as a bounded fallback when a real high-churn orientation gap exists.
...
Next recommended action: execute fixture-scoped `MRTR` on repository truth while preserving the current live OAuth21 `3008` runtime. `CIMD` is repo-complete but not live-loaded; `COMP-1A` follows only when newer external client-entry evidence exists, while `OPS-1B` and `DOC-2A` remain event-gated or bounded fallback work.
