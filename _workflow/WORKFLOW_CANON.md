...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=306`.
...
- `RETR-1-QUALITY-REGRESSION` remains accepted. The refreshed live v3 knowledge index contains `287` documents, visited `1237` files and `48` directories, reports freshness `fresh`, extracts all `14` readiness rows, and links all `36` source-of-truth documents through `757` internal links. Persisted samples remain bounded; complete canonical extraction is transient and bounded at `512 KiB`.
- Generator-owned directory orientation now covers `src/integrations`, `src/integrations/codebase_memory`, `_workflow/operator_decisions`, `.agents/skills/using-codebase-memory`, `.agents/skills/using-codebase-memory/references`, and the `docs/superpowers` plan/spec support boundary. `scripts/audit_directory_docs.js` ranks tracked high-churn directories by `DIRECTORY.md` coverage and currently reports no missing maps among the top 25 dirs with churn >= 5 in the last 30 days; this bounded `DOC-2A` fallback changes no runtime or connector contract.
...
- Workflow progress markers: `current_working_course = event-gated-compatibility-and-operational-quality`; `next_primary = comp-1a-on-fresh-external-client-traffic`; `next_secondary = ops-1b-on-live-sftp-boundary`.
...
1. Preserve the live-accepted Tasks/Trace/Artifacts/CIMD foundation and fixture-only `MRTR` as regression evidence; no additional protocol live-load action is pending.
2. Keep `COMP-1A` event-gated until operational Codex client-entry traffic newer than the August 17 `0.148.0-alpha.9` sample exists; do not reopen initialize retirement while that family remains `initialize_only`.
3. `OPS-1B` reconnect evidence is now accepted from the operator-driven OAuth reauthorization, connector refresh, modern `server/discover`, `98`-tool list, controlled restart, and post-restart callability. Only live SFTP remains open for OPS-1.
4. Use `DOC-2A` only when a fresh top25/churn>=5 audit exposes one real orientation gap; the August 16 audit is clean.
5. Run `COMP-1B` only if fresh `COMP-1A` evidence materially changes the retirement decision surface.
...
Next recommended action: keep the live-accepted `7 + 305` protocol regression line stable. `COMP-1A` reopens only on Codex client-entry traffic newer than the August 17 `0.148.0-alpha.9` sample; `OPS-1B` now waits only for a real live SFTP boundary, and `DOC-2A` remains closed while the top25/churn>=5 audit reports no missing map.
