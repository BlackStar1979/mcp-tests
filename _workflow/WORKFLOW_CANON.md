...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=301`.
...
- Workflow progress markers: `current_working_course = cimd-and-mrtr`; `next_primary = cimd`; `next_secondary = mrtr`.
...
2. Follow with the fixture-scoped `MRTR` conformance package without turning it into a second execution architecture.
...
Next recommended action: execute `CIMD` on repository truth with an explicit SSRF boundary while preserving DCR compatibility and the current live OAuth21 `3008` runtime. `MRTR` follows after the CIMD contract is stable; `COMP-1A`, `OPS-1B`, and `DOC-2A` remain event-gated or bounded fallback work.
