...
- `MEM-1` is live-loaded and full-suite green: append-only JSONL remains authoritative, optional OVH `bge-m3` embeddings use a non-authoritative hash-only SQLite cache, hybrid ranking preserves keyword fallback, egress is disabled unless provider/egress/token are all configured, and vectors or credentials never enter MCP output. On current runtime `2026-08-01T17:51:19.986Z`, `codex-mcp-client 0.146.0-alpha.9.2` fetched the current `673f28e12afea85c` descriptor fingerprint twice and own-connector `memory_search` succeeded. The `openWorldHint` descriptor review is closed without manual connector refresh; provider activation and PL/EN quality proof remain open.
...
- Workflow progress markers: `current_working_course = memory-embedding-runtime-activation-gate`; `next_primary = mem-1-live-provider-activation-and-quality-proof`; `next_secondary = mem-1-runtime-config-presence-audit`.
