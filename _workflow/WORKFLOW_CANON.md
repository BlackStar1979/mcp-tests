...
- Workflow progress markers: `current_working_course = event-gated-compatibility-and-operational-quality`; `next_primary = comp-1a-on-fresh-external-client-traffic`; `next_secondary = comp-1b-after-material-comp-1a-change`.
...
3. Preserve accepted `OPS-1` at `4/4`: OAuth reconnect/recovery remains accepted, and the 2026-08-17 connector-visible TEST MCP live SFTP list/read/`opsRoot` status closed `OPS-1B` without remote mutation, credential migration, runtime restart, or connector refresh.
...
Next recommended action: keep the live-accepted protocol, completed DOC-2, and accepted OPS-1 regression lines stable. `COMP-1A` is the only event-gated package and reopens only on Codex client-entry traffic newer than the August 17 `0.148.0-alpha.9` sample; `COMP-1B` runs only after a material change in that evidence.
