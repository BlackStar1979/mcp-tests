...
- `PROC-1A` is accepted and live on the current `89`-tool runtime. `PROC-1B` is repository-accepted and awaiting controlled live deployment: SQLite WAL persists owner-scoped job metadata, bounded output, and append-only transitions; completed jobs survive restart; orphans become `interrupted`; commands are never replayed; args/env are not persisted; process domain failures return controlled envelopes; Windows `py`, `powershell`, and `pwsh` resolver regressions are covered. Official-client OAuth E2E recovers the same job after process restart.
