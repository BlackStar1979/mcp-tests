...
- `RETR-1-QUALITY-REGRESSION` remains accepted. The refreshed live v3 knowledge index contains `279` documents, visited `1207` files and `51` directories, reports freshness `fresh`, extracts all `14` readiness rows, and links all `36` source-of-truth documents. Persisted samples remain bounded; complete canonical extraction is transient and bounded at `512 KiB`.
