...
- Fresh `COMP-1A` evidence now separates the stale latest runtime slice from the newest real entry window. The latest live `server_start_id = 2026-07-28T16:05:12.562Z` has follow-up traffic only; `--latest-entry-window` selects `server_start_id = 2026-07-28T03:49:00.666Z`, where operational `codex-mcp-client 0.146.0-alpha.3.1` on protocol `2025-06-18` remains `initialize_only`, with `2` successful legacy responses and `0` `server/discover` entries. Test child-server audit paths are hermetic, historical synthetic starts remain preserved, and the compatibility shim remains required.
