...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=305`.
...
- `OBS-1-LIVE-TRAIL` is accepted at `4/4`: the current post-restart window was correctly classified as stale entry evidence despite `11` successful follow-up tool calls, retained windows consistently identified the operational Codex initialize blocker, and paired response evidence remained available without raw payload logging. Follow-up CLI-integrity sweeps centralize fail-closed parsing across workflow probes, the operational soak runner, restart requests, OAuth pruning, marker-based patching, and runtime-log replacement; rejected arguments cannot silently alter scope or reach high-impact mutation behavior.
