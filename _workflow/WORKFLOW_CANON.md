...
- Policy coverage remains active as `POL-1 = 2/4`. The critical-policy reconciliation now proves `13/24` required policies implemented: transport security rejects invalid present `Origin` before dispatch under RED→GREEN evidence; OAuth21/internal tool dispatch enforces granted `mcp:tools`; and session security is reconciled to the stateless MCP `2026-07-28` target. `11` required policies remain outside `implemented`: `4` partial, `4` specified-only, and `3` critical (`consent`, `prompt/content`, `output DLP`). The earlier configured-root fix remains live-loaded from commit `555fad0`; the new transport/scope runtime changes are repository-applied and pending controlled OAuth21 `3008` live load.
