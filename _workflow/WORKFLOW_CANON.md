...
- Live OAuth21 `3008` and the connector enumerate tool count `84`, with current live proof `server_start_id = 2026-07-28T04:54:53.586Z`; the connector-visible hardened CBM v0.9.0 contract, including `state_handle`, partial-result metadata, semantic-only suppression, and source-bearing excluded-route warnings, is live and verified without connector-visible surface change.
- CBM reliability hardening is live: stdin payload transport emits no raw-JSON deprecation warning, `detect_changes` normalizes duplicate paths and exposes exact returned/omitted totals, `bridge_analysis`, and `impact_resolution_reason`, `ingest_traces` exposes `runtime_edge_creation` and `runtime_edges_created`, semantic-only `search_graph` suppresses unfiltered structural `results`, `index_repository` warns on known source-bearing excluded route directories, runtime output configuration fails fast, executable resolution is operator-neutral, ADR snapshot gating fails closed, and all three active cache databases are v0.9.0-compatible. Post-refresh delete verification returned `deleted` then `cbm_project_not_found`; no restart or connector refresh remains pending.
...
- Project-local CBM operating guidance is installed at `.agents/skills/using-codebase-memory/SKILL.md` and guarded by `_tests/smoke_cbm_agent_skill.js`; it now tells agents to prefer structured `bridge_analysis`, `impact_resolution_reason`, `runtime_edge_creation`, semantic-only suppression metadata, and `source_bearing_excluded_dirs` over warning prose. First-50 upstream issue review is recorded in `docs/CBM_UPSTREAM_ISSUES_FIRST50_REVIEW.md`.
- Generator-owned directory orientation now covers `src/integrations` and `src/integrations/codebase_memory`. The child map identifies native transport, contract registry, runtime orchestration, versioned contracts, and the repository/runtime/index truth boundary; this bounded `DOC-2A` fallback changes no runtime or connector contract.
- Fresh `COMP-1A` evidence explicitly selects live `server_start_id = 2026-07-27T03:10:26.042Z`: operational `openai-mcp 1.0.0` on protocol `2025-11-25` remains `initialize_only`, with `8` successful legacy responses and `0` `server/discover` entries. Test child-server audit paths are now hermetic, historical synthetic starts remain preserved, and the compatibility shim remains required.
