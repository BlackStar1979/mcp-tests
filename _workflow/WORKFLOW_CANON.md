...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=270`.
...
- Live OAuth21 `3008` and the connector enumerate tool count `84`, with current live proof `server_start_id = 2026-07-29T19:28:19.541Z`; the connector-visible hardened CBM v0.9.0 contract, including `state_handle`, partial-result metadata, semantic-only suppression, source-bearing exclusion warnings, Windows path/project caveats, Cypher shape caveats, and snippet source-integrity validation, is live and verified without tool-name or schema change.
...
- `MEM-1` is live-loaded and full-suite green: append-only JSONL remains authoritative, optional OVH `bge-m3` embeddings use a non-authoritative hash-only SQLite cache, hybrid ranking preserves keyword fallback, egress is disabled unless provider/egress/token are all configured, and vectors or credentials never enter MCP output. On current runtime `2026-08-01T17:51:19.986Z`, `codex-mcp-client 0.146.0-alpha.9.2` fetched the current `673f28e12afea85c` descriptor fingerprint twice and own-connector `memory_search` succeeded. The descriptor review is closed without manual connector refresh. A secret-free live PID `444` audit found provider, egress, token, and timeout override absent, with no embedding cache; activation is therefore safely blocked. The next package adds supervisor-compatible secret-file activation before live PL/EN quality proof.
...
- Fresh `COMP-1A` evidence separates current follow-up traffic from the newest real entry window. The current live `server_start_id = 2026-07-29T18:53:50.174Z` has no newer external entry evidence; `--latest-entry-window` selects `server_start_id = 2026-07-28T03:49:00.666Z`, where operational `codex-mcp-client 0.146.0-alpha.3.1` on protocol `2025-06-18` remains `initialize_only`, with `2` successful legacy responses and `0` `server/discover` entries. Test child-server audit paths are hermetic, historical synthetic starts remain preserved, and the compatibility shim remains required.
...
- Workflow progress markers: `current_working_course = memory-embedding-runtime-activation-gate`; `next_primary = mem-1-live-provider-activation-and-quality-proof`; `next_secondary = mem-1-secret-file-activation-package`.
...
1. Activate `MEM-1` only with server-owned OVH credentials plus explicit egress opt-in, then prove PL/EN ranking and failure fallback before backfill.
2. Keep `COMP-1A` event-gated until new external client traffic creates a meaningfully new evidence window.
...
Completed repair chain: `CBM-ADR-REPAIR`, `OAUTH-DUPLICATE-HELPER-REVIEW`, `FINAL-LIVE-LOAD`, `CBM-BRIDGE-SAMPLE-STRESS`, and `CBM-SNIPPET-INTEGRITY`. Restart `manual-1785348037500` loaded the final package at `server_start_id = 2026-07-29T18:00:39.162Z`; live connector proof recovered `searchIndex` from verified repository lines `1073-1084` while preserving native lines `898-909`.

Next recommended action: configure and validate `MEM-1` on the supervisor-managed OAuth21 `3008` runtime without changing the 84-tool count. Do not rerun `COMP-1A` until traffic newer than the July 28 `codex-mcp-client 0.146.0-alpha.3.1` entry window creates a meaningfully new entry-path sample. No public `3009` start is required.
