...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=312`.
- Latest validated public section count: `7`.
- Latest validated authenticated smoke count: `312`.
...
- Policy coverage remains `POL-1 = 2/4` with `15/24` required policies implemented. Current protocol/SEP authority is `SERVER_PROTOCOL_CAPABILITY_SPEC.json`, backed by `src/runtime/protocol_capability_registry.js`; capability-adaptive `consent_policy` is live accepted, while `prompt_content_policy` remains the critical gap.
- Standards cleanup, production MRTR, and capability-adaptive `POL-1A-CONSENT` remain live. OAuth21/internal currently runs source `cc75a66` at `server_start_id = 2026-08-20T17:34:29.129Z`; acceptance was established at `2026-08-20T17:24:29.112Z` by restart `manual-1787246667424`. `tools/list` remains `98` with fingerprint `93721a82a339f9d6`. Direct ChatGPT connector probes proved ordinary `run_process`, durable `process_start`, and bounded `write_file` without `elicitation.form`, while an unapproved process command remained rejected by the runner allowlist. `restart_required_now = false`; `connector_refresh_required_now = false` because tool names and descriptor/input/output schemas are unchanged.
...
- Workflow progress markers: `current_working_course = protocol-capability-module-convergence`; `next_primary = pol-1a-prompt-content`; `next_secondary = pol-1b`.
...
1. Execute `POL-1A-PROMPT` to close the untrusted-content/instruction-promotion boundary; capability-adaptive `POL-1A-CONSENT` authorization is live accepted.
2. Follow with `POL-1B` and `POL-1C`; keep `COMP-1A` event-gated until newer real Codex entry traffic exists.
3. Preserve production MRTR, live-accepted capability-adaptive `POL-1A-CONSENT`, `OPS-1`, live Tasks/Trace/Artifacts/CIMD, completed `DOC-2`, and the modular protocol-registry boundary as regression lines; preserve the historical process-MRTR acceptance as generic MRTR evidence.

Next recommended action: execute `POL-1A-PROMPT-CONTENT`, then continue bounded partial-policy reconciliation only after the remaining critical prompt/content boundary is closed.
