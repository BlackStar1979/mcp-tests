...
- Test processes are isolated from operational control state: self-test skips restart-controller startup and tool-surface persistence; every smoke child-server uses `_tests/helpers/hermetic_server_control_env.js`; standalone harnesses, ordinary self-test, and full `run_all` preserve the restored 84-tool `_control/tool-surface-state.json` byte-for-byte. Runtime snapshot smoke execution removes only its own uniquely labelled direct-child fixture in `finally`, and the full-suite no-pollution guard verifies that the retained snapshot directory set is unchanged. The one-time cleanup removed `684` accumulated smoke artifacts while preserving `17` named operational snapshots. These corrections affect test processes only and require no live-server restart.
