...
- Live OAuth21 `3008` is aligned with the authenticated `/mcp` tool count `95`; the final modular parity batch is now live-loaded after a controlled restart. Current live proof is server_start_id `2026-07-10T10:02:19.395Z`.
- Codex UI connector-side visible-tool enumeration was not independently re-verified in this turn, so current live `95` proof is runtime/MCP truth only, not a fresh UI inventory proof.
