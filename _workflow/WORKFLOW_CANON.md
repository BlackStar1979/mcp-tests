...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=253`.
...
- Workflow progress markers: `current_working_course = post_53d-initialize-compatibility-debug-and-retirement-scope`; `next_primary = post_53d-real-client-entry-evidence-refresh`; `next_secondary = post_53d-retirement-decision-delta-if-evidence-changes`.
...
1. Close one real high-churn documentation gap while protocol evidence waits on fresh external client traffic.
2. Preserve bounded single-route compatibility for both legacy `initialize` and modern `server/discover` until tested clients stop requiring legacy `initialize`, then only after explicit authorization retire `initialize`.
3. Prepare the bounded initialize-retirement decision package only after repo/runtime/client evidence stays coherent.

Next recommended action: keep the now-live `69`-tool OAuth21 `3008` connector-visible surface stable and wait for new external client traffic before rerunning `COMP-1A`. The bounded `DOC-2A` fallback is now complete: directory-map coverage explicitly reaches the runtime-owned control-plane backup roots and the live OAuth21 prune backup bundle without widening into repo-wide documentation churn. Modular parity against `C:\Work\mcp` is now repo-complete and live-loaded, so do not reopen `/mcp/sessionless` target planning as a fallback. Repo-side request flow is already stronger than the old blocker wording implied: the surviving `/mcp` route already supports `server/discover`, and active `tools/list` / `tools/call` already work without a preceding `initialize`. Fresh repo-native evidence from `2026-07-17` still leaves the blocker in place: the current audit window on `server_start_id 2026-07-17T17:38:34.244Z` shows `codex-mcp-client 0.145.0-alpha.18` entering through legacy `initialize` only, with `2` successful initialize responses and no matching fresh `server/discover` entry in that same current window, while the retained `1d` and `2d` operational windows already show the same blocker line. Earlier stale-entry windows still matter because client-entry observability now distinguishes follow-up-only `tools/list` / `tools/call` traffic from a real reconnect, so those slices still do not count as migration proof either way. The compatibility blocker is therefore specifically the entry path, not a broader authenticated request-flow failure after auth succeeds. The internal runtime helpers are now explicitly hidden from MCP schema/tools-list and no longer belong to connector-visible inventory. The first bounded residual cleanup is already repo-applied: unreachable GET-SSE, SessionStore, replay-helper, and push-emitter files were removed from the active repo while stable `/mcp` remained unchanged. The inactive sampling-context injection is now detached from active `/mcp` request handling, the remaining local helpers are explicitly classified as compatibility fixtures rather than active surviving-route wiring, and deprecated roots/sampling/logging inventory no longer remains unknown. The hidden-route retirement package is already repo-applied and live-verified. The bounded response-side observability package is now repo-applied, live-loaded, and bounded-live-verified on OAuth21 `3008`: active `/mcp` runtime emits `rpc_response_sent` for standard JSON-RPC responses and early-exit rejection paths, and retained real-client traffic still confirms the full `initialize -> notifications/initialized -> tools/list -> tools/call` chain. Recent repo-only OAuth21 hardening also closed the public DCR/body abuse paths without changing the surviving `/mcp` contract. Connector refresh remains unnecessary because surviving `/mcp` is unchanged. Public 3009 start remains unnecessary unless a narrower record says otherwise.
