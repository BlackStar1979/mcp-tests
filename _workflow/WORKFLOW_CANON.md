...
- Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok=true, version=0.40.0, public=7, tests_authenticated=308`.
...
- `MRTR` is repo-validated and closed as fixture-only conformance evidence at `7 + 305`. It deliberately has no production runtime surface, public tool, or second task/session store; runtime loading is therefore not applicable. The controlled restart preserved the `98`-tool surface and did not introduce any MRTR runtime object.
...
- `RETR-1-QUALITY-REGRESSION` remains accepted. The refreshed live v3 knowledge index contains `288` documents, visited `1238` files and `48` directories, reports freshness `fresh`, extracts all `14` readiness rows, and links all `36` source-of-truth documents through `758` internal links. Persisted samples remain bounded; complete canonical extraction is transient and bounded at `512 KiB`.
...
- Policy coverage remains active as `POL-1 = 2/4`. Current repository and live-runtime truth proves `14/24` required policies implemented: transport security rejects invalid present `Origin`; OAuth21/internal tool dispatch enforces granted `mcp:tools`; session security matches the stateless MCP `2026-07-28` target; and output DLP enforces JSON Schema 2020-12 validation, secret redaction, untrusted-output metadata, resource-link filtering, and embedded-resource handling across optional tools, core search/fetch, and process Tasks. `10` required policies remain outside `implemented`: `4` partial, `4` specified-only, and `2` critical (`consent`, `prompt/content`). Final DLP source commit `b385c88` is live on OAuth21 `3008` at `server_start_id = 2026-08-18T04:39:45.722Z`; health is `200`, the live surface is `98` tools with output `8292895f0216967c`, descriptor `82ffaaa3adb6e7db`, combined `93721a82a339f9d6`, and durable process recovery remains green. A fresh reconnect exposed that the POL-1A `Origin` gate was applied globally and blocked `/oauth/operator-login`. Source commit `2ae1585` scopes the check to `/mcp` and is live at `server_start_id = 2026-08-18T14:53:56.306Z`; live validation preserves `403 Invalid Origin` on `/mcp` while the OAuth operator-login POST reaches its handler. The earlier operator refresh is independently server-audited: `openai-mcp 1.0.0` completed `server/discover` and `tools/list` with `98` tools and fingerprint `93721a82a339f9d6`. Runtime, repository, and model-visible connector schemas are therefore aligned; `restart_required_now = false` and `connector_refresh_required_now = false`.
- Workflow progress markers: `current_working_course = policy-coverage-critical-gap-closure`; `next_primary = pol-1a-consent-boundary`; `next_secondary = pol-1a-prompt-content-boundary`.
...
- Earlier checkpointed hygiene closeout is complete. Earlier checkpoint-specific implementation boundaries remain historical evidence unless reaffirmed by later records.
...
- Workflow-rule correction: workflow rules must be guarded by smoke, not merely documented.
...
1. Refresh/review the authenticated ChatGPT connector so its model-visible output schemas match the accepted live DLP runtime fingerprint `93721a82a339f9d6`; this is an external operator/UI checkpoint, not another runtime implementation package.
2. Execute `POL-1A-CONSENT`: require a server-verifiable consent artifact for high-risk runtime operations instead of treating `guarded_process_execution` as sufficient proof.
3. Execute `POL-1A-PROMPT`: classify content-bearing tool results as untrusted data by default and add an explicit instruction-promotion boundary.
4. Follow with `POL-1B` for the four `partial` policies and `POL-1C` for the four non-critical `specified_only` policies.
5. Preserve the live-accepted Tasks/Trace/Artifacts/CIMD foundation, fixture-only `MRTR`, accepted `OPS-1`, and completed `DOC-2` regression lines.
6. Keep `COMP-1A` event-gated until real Codex client-entry traffic newer than `0.148.0-alpha.9` exists; run `COMP-1B` only if fresh evidence materially changes initialize-retirement.
Completed repair chain: `CBM-ADR-REPAIR`, `OAUTH-DUPLICATE-HELPER-REVIEW`, `FINAL-LIVE-LOAD`, `CBM-BRIDGE-SAMPLE-STRESS`, `CBM-SNIPPET-INTEGRITY`, `RETR-1-QUALITY-REGRESSION`, and `DEBT-1-RETIREMENT`. Controlled restart `manual-1786939596474` loaded the final cleanup at `server_start_id = 2026-08-17T04:06:37.912Z`; live callability, the `98`-tool surface, and fingerprint `ec7d3af5b4ea17f5` are accepted without OAuth relogin or connector refresh.

Next recommended action: execute `POL-1A` while preserving accepted runtime/protocol/OPS/DOC behavior. `COMP-1A` remains an independent event-gated lane and reopens only on real Codex client-entry traffic newer than `0.148.0-alpha.9`; `COMP-1B` remains conditional.
