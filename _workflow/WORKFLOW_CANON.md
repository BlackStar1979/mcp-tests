...
- Policy coverage remains `POL-1 = 3/4`. Repository target coverage is now `20/24` implemented after `POL-1B`, with only four `specified_only` rows left for `POL-1C`; the currently loaded OAuth21 runtime remains on the accepted `16/24` baseline until the pending controlled restart. All `6/6` critical policies remain live accepted. Current protocol/SEP authority is `SERVER_PROTOCOL_CAPABILITY_SPEC.json`, backed by `src/runtime/protocol_capability_registry.js`.
- Standards cleanup, production MRTR, capability-adaptive `POL-1A-CONSENT`, and `POL-1A-PROMPT-CONTENT` remain live. OAuth21/internal currently runs closeout commit `3acaefe7a4df6cd0557076d634776499418893e9` at `server_start_id = 2026-08-21T19:27:12.690Z` after restart `manual-1787340431244`; that commit retains the accepted prompt/content behavior source `20ab92155dfb9ba356d079f7a9264e7d8cf67005`. `tools/list` remains `98` with fingerprint `93721a82a339f9d6`. Raw authenticated safe and hostile tool-call probes prove the model-visible untrusted-content boundary while preserving primary data and `structuredContent`; the hostile fixture remains data-only with promotion forbidden. `restart_required_now = false`; `connector_refresh_required_now = false` because tool names and descriptor/input/output schemas are unchanged.
...
- Workflow progress markers remain `current_working_course = protocol-capability-module-convergence`; `next_primary = pol-1b`; `next_secondary = pol-1c` until `POL-1B` is live accepted.
...
1. Complete the repo-validated `POL-1B` live load and proof; do not advance the marker until the controlled restart confirms the new network/quota runtime behavior and unchanged connector surface.
2. Follow with `POL-1C`; keep `COMP-1A` event-gated until newer real Codex entry traffic exists.
...
Next recommended action: complete `POL-1B` live acceptance, then advance to bounded `POL-1C` reconciliation for the four `specified_only` non-critical policies.
