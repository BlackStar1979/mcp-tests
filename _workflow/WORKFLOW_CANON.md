...
- Policy current-truth reconciliation is active as `POL-1 = 2/4`. Canonical policy coverage moved from `6/24` to `10/24` implemented after evidence-based reconciliation; `14` required policies remain outside `implemented`. A TDD probe reproduced and repo-fixed configured-root symlink/junction escape in central `safeWorkspacePath()`. The runtime-imported fix is not live on OAuth21 `3008` until controlled supervisor restart; no connector refresh is required.
