...
| RETR-1 | Workspace retrieval ergonomics | 3/4 | Reduce agent execution friction without changing product truth | DOC-1, SURF-1 | `workbench` callability and repository indexing are materially better than before. The workspace retrieval index is now a documentation/knowledge helper by default: `profile=knowledge` indexes documentation and structured project-truth files, excludes code-like files and noisy control-plane records, and preserves `profile=source`/`profile=all` for explicit exceptions. Read tools expose shared retrieval metadata (`index_scope`, `path_filter`, `index_profile`, `index_truncated`, `index_created_at`, `index_count`) and each result includes `title`, `kind`, `authority`, and `format`. `build_index` and `index_status` expose `knowledge_summary` facets plus `workflow_summary`, including canonical document presence, runtime identity, workflow markers, readiness components, roadmap items, server-spec count, health flags, and documentation gaps. Validation on July 28, 2026: scoped `mcp-tests` knowledge build completed with `truncated=false`, `247` indexed documents, `1099` visited files, `852` extension skips, `10` readiness components, `6` roadmap items, `29` server specs, runtime identity `mcp-tests-response-shape 0.40.0`, and no canonical documentation gaps. | Retrieval quality still depends on environment freshness, JSON snapshot size, and ranking heuristics; operator-decision records remain numerous but are now classified instead of mixed with code. | Keep the default index as a knowledge/documentation map; use CBM for code analysis; use `profile=source` or `profile=all` only for explicit diagnostics; move to SQLite/FTS only if JSON snapshot size or ranking quality becomes a reproduced blocker. | Retrieval issues stop being a recurring blocker and the index answers project-orientation questions without competing with CBM. |
