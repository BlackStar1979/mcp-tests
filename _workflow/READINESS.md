...
| RETR-1 | Workspace retrieval ergonomics | 2/4 | Reduce agent execution friction without changing product truth | DOC-1, SURF-1 | `workbench` callability and repository indexing are materially better than before. The workspace retrieval index was rebuilt on July 28, 2026 and exposed a real scale boundary: `C:\Work` reaches the `max_files=50000` cap, so `index_status` now reports persisted scan stats including `truncated`, limits, visited counts, and skipped counts. | Retrieval quality still depends on environment freshness, scan caps, and ranking heuristics. | Improve only when it materially shortens current target work or reduces evidence-gathering friction. | Retrieval issues stop being a recurring blocker on active packages. |
...
- `2026-07-28`: tightened workspace retrieval index diagnostics after live `workbench.index_status` showed only `status=ok` for a stale index while `build_index` rebuilt `C:\Work` with `truncated=true` at the `max_files=50000` cap. `index_status` now returns persisted scan diagnostics (`visited_files`, `visited_dirs`, `truncated`, `max_files`, `max_dirs`, `skipped`) so agents can distinguish a healthy complete index from a healthy partial index. `_tests/smoke_build_index_tool.js`, `_tests/smoke_output_schema_guard.js`, and `_tests/smoke_schema_compat.js` guard the contract.
