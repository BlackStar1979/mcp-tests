...
| RETR-1 | Workspace retrieval ergonomics | 4/4 | Reduce agent execution friction without changing product truth | DOC-1, SURF-1 | `workbench` callability and repository indexing are materially better than before. The workspace retrieval index is now a documentation/knowledge helper by default: `profile=knowledge` indexes documentation and structured project-truth files, excludes code-like files and noisy control-plane records, and preserves `profile=source`/`profile=all` for explicit exceptions. Read tools expose shared retrieval metadata (`index_scope`, `path_filter`, `index_profile`, `index_truncated`, `index_created_at`, `index_count`) and each result includes `title`, `kind`, `authority`, and `format`. `build_index` and `index_status` expose `knowledge_summary` facets plus `workflow_summary`, including canonical document presence, runtime identity, workflow markers, readiness components, roadmap items, server-spec count, health flags, and documentation gaps. Validation on July 29, 2026: scoped `mcp-tests` knowledge build completed with `truncated=false`, `251` indexed documents, `1103` visited files, `852` extension skips, `10` readiness components, `6` roadmap items, `29` server specs, runtime identity `mcp-tests-response-shape 0.40.0`, and no canonical documentation gaps. Natural workflow-orientation queries now rank `_workflow/ACTIVE_WORKFLOW_INDEX.md`, `_workflow/READINESS.md`, and `_workflow/ROADMAP.md` ahead of historical operator decisions in both `search_index_context` and `collect_context`; guarded by `_tests/smoke_build_index_tool.js`. | No active retrieval blocker remains for the current documentation/workflow use case. SQLite/FTS remains a future scale option only if JSON snapshot size or ranking quality becomes a reproduced blocker. | Keep the default index as a knowledge/documentation map; use CBM for code analysis; use `profile=source` or `profile=all` only for explicit diagnostics; preserve active-workflow intent ranking. | Project-orientation questions return active workflow truth before historical decision records, without competing with CBM. |
...
`CBM-1`, `SURF-1`, and `RETR-1` have no active blocker. Runtime callability and UI-visible enumeration remain separate evidence layers, but the current hardened 84-tool identity is stable.
