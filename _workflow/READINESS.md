...
| CBM-1 | Codebase-Memory bridge and index integrity | 4/4 | Keep repository analysis trustworthy and mutation-safe | DOC-1, SURF-1 | Stdin native transport, duplicate change normalization, fail-fast runtime output configuration, operator-neutral executable fallback, fail-closed ADR snapshot gating, exact OAuth/HTTP helper consolidation, and v0.9.0 cache-schema repair are live at `server_start_id = 2026-07-27T03:10:26.042Z`. All three active indexes retain their counts; `manage_adr(get)` succeeds; live `detect_changes` removed 68 duplicate entries; full offline smoke is GREEN at `7 + 266`. Manual multi-repo stress now covers default and large sample sets, with `1057` repeated bridge calls across `8` sample-repo indexes and `instability=[]` after filtering volatile native search-latency warnings from `cbm_search_code` results. | No active CBM integrity blocker remains. | Preserve the backup, migrator, and stress harness; reopen CBM work only on a reproduced regression or approved capability change. | Live identity remains 84 tools with unchanged fingerprints, repaired ADR storage, normalized and stable stress-tested results, and no restart or connector refresh pending. |
...
- `2026-07-28`: completed manual CBM bridge sample-repo stress closeout. `_tests/stress_cbm_bridge_samples.js` validates all exposed CBM bridge tools repeatedly across selected `_repos_with_code_samples`; the default four-repo run produced `528` calls with `instability=[]`, and the large `OVH,useable,docs-develop,NousResearch` run produced `529` calls with `instability=[]` after fixing volatile `cbm_search_code` warning normalization. Full offline smoke is GREEN at `7 + 266`; no connector-visible surface change was made.
