...
const MARKER = "s13_connector_migration_dry_run_harness";
...
function buildHarnessResult() {
  ...
  const s10bRecord = read("_workflow/operator_decisions/s10b_live_authenticated_sessionless_probe.md");
  const s11Record = read("_workflow/operator_decisions/s11_connector_migration_readiness_coexistence_boundary.md");
  const s12Record = read("_workflow/operator_decisions/s12_connector_migration_dry_run_plan.md");
  ...
  const candidateRoute = String(runtimeConfig.sessionless_prototype.route || "");
  const toolNames = [
  ...
  assert.ok(runtimeConfig.http_routes.includes("/mcp/sessionless"));
  ...
  const result = {
    ...
    preconditions: {
      ...
      stable_session_code_removal_forbidden: state.current_work_constraints.do_not_remove_stable_session_code_before_S2_target_selection === true,
