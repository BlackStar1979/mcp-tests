...
| P2 | Preserve the hardened CBM and 84-tool surface | stable live runtime | The repaired bridge, indexes, connector identity, manual multi-repo stress harness, upstream issue reviews through ordinal 277, and snippet source-integrity recovery are production support truth. | Reopen CBM work only on a reproduced regression, failed stress run, new upstream issue with local impact, or approved capability change. |
...
| P5 | Keep OAuth21 runtime hardening bounded and explicit | live canonical PKCE validation | Prevent auth controls from drifting back into permissive parsing or unaudited rejection behavior. | Preserve the new PKCE boundary and audit guard; reopen auth only on reproduced evidence or a current standards delta. |
