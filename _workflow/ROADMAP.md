...
- `COMP-1` remains the highest-leverage open blocker.
- `SURF-1` should move only when it changes the `COMP-1` decision.
...
| P2 | Compare real client behavior against live authenticated observability | stable OAuth21 auth, live-loaded observability package, confirmed local authenticated probe | This is the shortest path to higher-confidence `initialize` retirement evidence. | Focus on real Codex/OpenAI `initialize` compatibility paths, not more synthetic local probes. This is the default next execution package while `COMP-1` stays blocked. |
