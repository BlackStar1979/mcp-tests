...
- `RETR-1` retrieval now exposes workflow facts directly, rejects silent staleness, parses complete canonical workflow documents through a bounded transient sample, and ranks authoritative current evidence ahead of historical records. The refreshed live v3 knowledge index contains `266` documents, visited `1157` files and `50` directories, reports freshness `fresh`, extracts all `13` readiness components, and links all `36` source-of-truth documents.
