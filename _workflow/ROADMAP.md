...
- `DOC-1` is `4/4`: a fresh single `index_status` response carries workflow markers, component maturity/blocker/default-package data, ordered roadmap actions, runtime identity, canonical health, and documentation gaps. Live planning, maturity, and direct-proof queries rank the intended evidence first.
...
| P5 | `POL-1A` critical policy gap closure — active | canonical policy matrix, current runtime/tests | Current repository and live-runtime truth proves `14/24` required policies implemented; transport, session, scope-minimization, and output DLP are evidence-backed, while consent and prompt/content remain genuine critical runtime gaps. Final DLP source commit `b385c88` is live on OAuth21 `3008` at `2026-08-18T04:39:45.722Z`. A fresh reconnect exposed that the global MCP Origin gate also blocked `/oauth/operator-login`; the repo fix scopes Origin validation to `/mcp` and now requires a controlled restart before the already-pending external connector refresh/review. | Refresh/review the authenticated connector, then execute `POL-1A-CONSENT`; close prompt/content as a separately testable package before `POL-1B`. |
...
0. `POL-1A-DLP` — runtime live accepted; OAuth Origin regression fix restart pending; connector refresh/review pending
   Final DLP source commit `b385c88` is live on OAuth21 `3008` at `server_start_id = 2026-08-18T04:39:45.722Z`; health, durable process recovery, live `read_file`, DLP regression coverage, and the `98`-tool runtime fingerprint `93721a82a339f9d6` are green. A fresh reconnect exposed `403 Invalid Origin` because the MCP Origin gate was applied to `/oauth/operator-login`; the repo fix scopes the gate to `/mcp` and must be live-loaded before retrying OAuth. The connector/model cache still carries `ec7d3af5b4ea17f5`, so connector refresh/review remains required after that restart.
...
Completed mutating CLI root fidelity: relative marker-patch targets, replacement files, runtime logs, and compaction outputs resolve from the repository root even when invoked from another cwd; explicit absolute paths remain supported and functional foreign-cwd tests guard both mutation paths.
Completed read-only control-plane root fidelity: shared root-spec loading plus active fixture validators, decision evaluators, negative controls, decision-runtime contract validators, and index-authority reporting resolve repository truth independently of caller cwd; explicit absolute spec inputs remain supported.
Completed deploy-decision runtime-boundary fidelity: guard v2 normalizes relative, absolute, and Windows-style paths; recognizes startup-loaded source, tool, profile, plugin, server-spec, and package-manifest changes; preserves repo-only documentation/tests and compatibility-metadata specialization; and is live-accepted on the unchanged `98`-tool surface after controlled restart `manual-1786980050739`.
Completed public-sandbox mutator retirement: the unguarded historical source-copy script is removed from the active control plane, its stale smoke remains archived as provenance, and a current regression prevents accidental reintroduction of the duplicate-truth path.
Completed snapshot and full-harness root fidelity: snapshot creation is caller-cwd independent, stages content under a unique `.pending-*` directory, retries only transient Windows same-volume rename failures, and rolls staging back on terminal failure. `run_all_smokes.js` anchors server, patch, sandbox, child-process, and relative-manifest paths to the repository; the no-pollution guard executes the inner full suite from a foreign cwd, verifies an unchanged snapshot directory set, and `684` accumulated smoke artifacts were removed while preserving `17` named operational snapshots.
