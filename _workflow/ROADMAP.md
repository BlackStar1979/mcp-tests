...
- `RETR-1` retrieval now exposes workflow facts directly, rejects silent staleness, parses complete canonical workflow documents through a bounded transient sample, and ranks authoritative current evidence ahead of historical records. The live v3 knowledge index contains `259` documents, visited `1122` files and `50` directories, reports freshness `fresh`, and passes active-workflow, component-maturity, and `MEM-1` proof queries.
