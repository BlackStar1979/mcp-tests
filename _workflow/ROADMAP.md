...
Completed observability and CLI-integrity acceptance: `OBS-1-LIVE-TRAIL` proves that bounded current-window and retained-window evidence answers live client-entry questions without broader logging. The follow-up sweeps centralize fail-closed parsing for workflow probes, the operational soak runner, restart requests, OAuth pruning, marker-based patching, and runtime-log replacement; malformed arguments cannot silently widen scope or reach high-impact mutation behavior.
