...
- `MEM-1` is repository-ready: append-only JSONL remains authoritative, optional OVH `bge-m3` vectors live in a hash-only SQLite sidecar, hybrid ranking is guarded, and provider/cache failure preserves keyword behavior. A secret-free live PID `444` audit proved provider, egress, and token are absent; runtime activation is safely blocked until a supervisor-compatible secret-file path and live PL/EN quality evidence exist.
...
| --- | --- | --- | --- | --- |
| P0 | Activate and validate `MEM-1` | secret-file activation package, server-owned OVH token, explicit egress opt-in | The retrieval implementation is complete; the current environment audit proves activation gates are absent. | Add the secret-file path first, provision the token without durable environment leakage, restart through the supervisor, run bounded PL/EN related/unrelated probes, and retain keyword fallback unless acceptance evidence passes. |
