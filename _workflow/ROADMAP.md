...
- The hardened CBM v0.9.0, canonical PKCE behavior, and durable process lifecycle are live in the current `91`-tool runtime; all fifteen `cbm_*` tools and verified snippet source-integrity recovery are unchanged.
...
| P1 | Re-enumerate the durable async process extension | live `91`, current model map `89` | The server is deployed; this task cannot gain the two new names after startup. | Re-enumerate the Codex environment for direct `process_list/events` acceptance without another server restart. |
