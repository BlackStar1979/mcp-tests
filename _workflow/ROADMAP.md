...
Completed observability and CLI-integrity acceptance: `OBS-1-LIVE-TRAIL` proves that bounded current-window and retained-window evidence answers live client-entry questions without broader logging. The follow-up sweep centralizes fail-closed parsing for workflow probes, the operational soak runner, and the restart-request helper; malformed arguments cannot silently widen scope or write a default restart trigger.
