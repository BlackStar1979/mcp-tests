...
- `MEM-1` is repository-ready: append-only JSONL remains authoritative, optional OVH `bge-m3` vectors live in a hash-only SQLite sidecar, hybrid ranking is guarded, and provider/cache failure preserves keyword behavior. Runtime activation remains blocked by absent server-owned configuration and live PL/EN quality evidence.
...
| --- | --- | --- | --- | --- |
| P0 | Activate and validate `MEM-1` | server-owned OVH token plus explicit egress opt-in | The implementation is complete and the remaining gap is measurable runtime retrieval quality, not speculative feature work. | Configure deployment environment, restart through the supervisor, run bounded PL/EN related/unrelated probes, and retain keyword fallback unless acceptance evidence passes. |
