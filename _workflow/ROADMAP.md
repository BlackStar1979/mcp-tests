...
- The hardened CBM v0.9.0 and canonical PKCE behavior remains live in the current `89`-tool runtime while the repository target advances additively to `91`; all fifteen `cbm_*` tools and verified snippet source-integrity recovery are unchanged.
...
- `RETR-1` retrieval now exposes workflow facts directly, rejects silent staleness, parses complete canonical workflow documents through a bounded transient sample, and ranks authoritative current evidence ahead of historical records. The live v3 knowledge index contains `262` documents, visited `1131` files and `50` directories, reports freshness `fresh`, extracts all `12` readiness components, and links all `36` source-of-truth documents.
...
- `PROC-1B` is repository-accepted: the target is `91`, jobs and transitions are durable in owner-scoped SQLite, terminal status/output survive restart, unclean orphans become `interrupted`, policy failures are controlled responses, and the `py`/PowerShell resolver defects from external stress testing are repaired. Controlled deployment and post-restart live recovery remain.
...
| P1 | Deploy and re-enumerate the durable async process extension | repo `91`, live `89` | Runtime must load SQLite persistence and the two discovery/history tools; this task cannot gain new names after startup. | Use the controlled restart helper, prove one job across a second restart, then re-enumerate the Codex environment for `process_list/events`. |
...
Completed process execution phase A: `PROC-1A` shares one hardened sync/async core and is live on the `89`-tool runtime after restart `manual-1786285267764`. Repository phase B adds durable recovery, rediscovery/history, controlled errors, and resolver fixes; deployment evidence will be recorded separately.
