...
- `DOC-2A` is now the active bounded fallback because the protocol-evidence path is waiting on fresh external client traffic, not missing repo work.
...
| P5 | Execute `DOC-2A` as the active bounded fallback package | P1 | Repo orientation still needs selective extension, and the higher-leverage protocol work is currently externally timing-bound. | Close one real high-churn documentation gap and keep the next package inferable from docs alone; avoid repo-wide documentation sprawl. |
...
1. `DOC-2A`
   Close one real high-churn documentation gap while the protocol-evidence path waits on fresh external client traffic.
