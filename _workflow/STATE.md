...
- Live OAuth21 runtime exposes `98` tools at `server_start_id = 2026-08-17T17:43:05.322Z` with governed fingerprint `ec7d3af5b4ea17f5`. Controlled restart `manual-1786988583476` loaded repository head `555fad0`; `/healthz` returned OAuth21/internal with `98` tools, the tool-surface audit saved the unchanged fingerprint, connector calls remained usable, and a durable pre-restart process record was recovered after restart.
...
- `POL-1` remains active at `2/4`: current repository truth now proves `14/24` required policies implemented. Transport security rejects invalid present `Origin`, OAuth21/internal tool dispatch requires granted `mcp:tools`, session security matches the stateless MCP `2026-07-28` target, and output DLP now centralizes JSON Schema 2020-12 validation, secret redaction, untrusted-output labels, resource-link filtering, and embedded-resource fail-closed handling across optional tools, core search/fetch, and process Tasks. Ten required policies remain non-implemented: four partial, four specified-only, and two critical (`consent`, `prompt/content`). Live OAuth21 `3008` still runs the previously accepted source commit `cbbb284` at `server_start_id = 2026-08-17T19:00:17.718Z`; the DLP runtime changes are repo-applied but not live-loaded yet, so `restart_required_now = true`. The live connector/runtime still exposes `98` tools with the previously accepted fingerprint `ec7d3af5b4ea17f5`, while DLP enforcement exposed and repaired ten impossible placeholder output schemas. Repository target fingerprints are now output `8292895f0216967c`, descriptor `82ffaaa3adb6e7db`, combined `93721a82a339f9d6`; tool names and input schemas are unchanged. Therefore `connector_refresh_required_now = true` in addition to the pending runtime restart.
