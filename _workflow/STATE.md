...
- `OBS-1` is live-accepted at `4/4`. On `server_start_id = 2026-08-17T04:06:37.912Z`, bounded diagnostics read the audit trail with zero parse errors, classified `11` follow-up tool calls without an entry event as `stale_entry_window`, and retained the Codex initialize blocker across `1d`/`2d`/`7d`/`30d`/`all`. Shared CLI parsing now covers workflow probes, the operational soak runner, the restart-request helper, OAuth prune control-plane, marker-based patching, and runtime-log replacement; both value forms are accepted, while malformed or conflicting arguments fail before evidence scope, restart triggers, OAuth records, target files, or raw logs can change.
