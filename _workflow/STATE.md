...
- Runtime snapshot smoke execution is hermetic: it validates the generated snapshot and removes only its own uniquely labelled direct-child directory in `finally`. The full-suite no-pollution guard verifies that the snapshot directory set is unchanged. A one-time cleanup removed `684` accumulated smoke artifacts while preserving all `17` named operational snapshots.
