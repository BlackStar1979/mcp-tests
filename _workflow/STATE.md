...
- Runtime snapshot creation stages all files and the manifest under a unique `.pending-*` directory, publishes with one same-volume rename, and removes staging on failure. The smoke validates both success cleanup and mid-copy rollback, then removes only its own uniquely labelled direct-child fixture in `finally`; the full-suite no-pollution guard verifies that the snapshot directory set is unchanged. A one-time cleanup removed `684` accumulated smoke artifacts while preserving all `17` named operational snapshots.
