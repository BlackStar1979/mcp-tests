...
- `POL-1` remains `3/4`. Repository target coverage is now `20/24` implemented after `POL-1B`; the currently loaded runtime remains at the previously accepted `16/24` policy baseline until the pending controlled restart. `network_policy` now has bounded streaming enforcement, `plugin_visibility_policy` is reconciled to the accepted pull-only/static surface target, `runtime_topology` records the restored supervisor authority without contradictory missing-authority debt, and `rate_limit_quota_policy` now partitions OAuth clients by SHA-256-derived identity and applies dedicated network/process ceilings. The only four repo rows outside `implemented` are `memory_policy`, `database_policy`, `supply_chain_policy`, and `incident_response_policy`, all `specified_only`. `POL-1B` is repo-validated but still live-pending; `POL-1C` begins only after live acceptance. `restart_required_now = true`; connector refresh is not required because the governed `98`-tool surface and fingerprints are unchanged.
