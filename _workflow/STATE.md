...
- Live OAuth21 runtime exposes `98` tools at `server_start_id = 2026-08-17T04:06:37.912Z` with governed fingerprint `ec7d3af5b4ea17f5`. Controlled restart `manual-1786939596474` loaded repository head `e32fefdb4d0f369a9ff30e1ecba7f94600807130`; OAuth state remained usable and direct `workbench.get_info` succeeded after restart.
...
- Runtime snapshot creation resolves repository scope from the script location rather than caller cwd, stages all files and the manifest under a unique `.pending-*` directory, publishes with one same-volume rename, and removes staging on failure. The smoke validates foreign-cwd fidelity, success cleanup, and mid-copy rollback, then removes only its own uniquely labelled direct-child fixture in `finally`; the full-suite no-pollution guard verifies that the snapshot directory set is unchanged. A one-time cleanup removed `684` accumulated smoke artifacts while preserving all `17` named operational snapshots.
