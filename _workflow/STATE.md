...
- Runtime snapshot creation resolves repository scope from the script location rather than caller cwd, stages all files and the manifest under a unique `.pending-*` directory, publishes with a bounded same-volume rename retry for transient Windows `EACCES`/`EBUSY`/`EPERM`, and removes staging on failure. The smoke validates foreign-cwd fidelity, deterministic transient-rename recovery, success cleanup, and mid-copy rollback, then removes only its own uniquely labelled direct-child fixture in `finally`. The full `run_all` orchestrator now resolves its server, auth patch, sandbox, child processes, and relative manifest from the repository root; the no-pollution guard executes that full inner run from a foreign cwd and verifies that the snapshot directory set is unchanged. A one-time cleanup removed `684` accumulated smoke artifacts while preserving all `17` named operational snapshots.
