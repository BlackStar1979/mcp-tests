...
- authorized runtime: `auth:oauth21`, port `3008`, current live and repository surface 84 connector-visible tools at `server_start_id = 2026-07-28T03:49:00.666Z`; current `COMP-1A` evidence remains the earlier July 27 `initialize_only` window for `openai-mcp 1.0.0`;
...
- Counts remain aligned at 84 and the hardened connector-visible CBM v0.9.0 contract is live at `server_start_id = 2026-07-28T03:49:00.666Z`; `state_handle` deletion, not-found classification, source preservation, normalized change results, partial-result metadata, repaired ADR storage, and unchanged connector-visible surface are verified. No restart or connector refresh remains pending.
