...
- authorized runtime: `auth:oauth21`, port `3008`, current live and repository surface 84 connector-visible tools at `server_start_id = 2026-07-28T15:25:30.678Z`; current `COMP-1A` evidence remains the earlier July 27 `initialize_only` window for `openai-mcp 1.0.0`;
...
- Counts remain aligned at 84 and the hardened connector-visible CBM v0.9.0 contract is live at `server_start_id = 2026-07-28T15:25:30.678Z`; `state_handle` deletion, not-found classification, source preservation, normalized change results, partial-result metadata, semantic-only suppression, source-bearing excluded-route warnings, Windows non-ASCII search caveats, Cypher shape caveats, repaired ADR storage, and unchanged connector-visible surface are verified. No restart or connector refresh remains pending.
