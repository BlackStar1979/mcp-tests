...
- authorized runtime: `auth:oauth21`, port `3008`, current live and repository surface 84 connector-visible tools at `server_start_id = 2026-08-01T17:51:19.986Z`; current `COMP-1A` evidence remains `initialize_only` on the July 28 latest-entry window for `codex-mcp-client 0.146.0-alpha.3.1`;
...
- Counts remain aligned at 84 and the hardened connector-visible CBM v0.9.0, canonical PKCE, and keyword-safe memory contracts are live at `server_start_id = 2026-08-01T17:51:19.986Z`; current Codex `tools/list` evidence, descriptor fingerprint `26f35b84a92470b8`, and direct memory callability close the descriptor-only review without a manual connector refresh.
