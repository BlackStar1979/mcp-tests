...
Snapshot and full-harness root fidelity are closed: snapshot creation is caller-cwd independent, stages files and manifest under a unique `.pending-*` directory, retries transient Windows rename locks without abandoning same-volume atomic publication, and rolls staging back on terminal failure. `run_all_smokes.js` anchors all repository resources and relative manifests to its script location; its no-pollution guard executes the inner full suite from a foreign cwd, verifies an unchanged snapshot directory set, and one-time cleanup removed `684` smoke artifacts while preserving `17` named operational snapshots. No runtime restart or connector refresh is required.
