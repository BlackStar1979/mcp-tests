...
- authorized runtime: `auth:oauth21`, port `3008`, current live and repository surface 84 connector-visible tools at `server_start_id = 2026-08-01T18:52:13.024Z`; current `COMP-1A` evidence remains `initialize_only` on the July 28 latest-entry window for `codex-mcp-client 0.146.0-alpha.3.1`;
...
- Counts remain aligned at 84 and the hardened connector-visible CBM v0.9.0, canonical PKCE, and activated memory contracts are live at `server_start_id = 2026-08-01T18:52:13.024Z`; direct PL/EN retrieval and lexical fallback survived the controlled restarts and no connector refresh is pending.
