...
Snapshot integrity and harness hygiene are closed: snapshot creation stages files and manifest under a unique `.pending-*` directory, publishes with one same-volume rename, and rolls staging back on failure. Runtime snapshot smoke execution removes only its own uniquely labelled direct-child fixture in `finally`, the full-suite no-pollution guard verifies an unchanged snapshot directory set, and one-time cleanup removed `684` smoke artifacts while preserving `17` named operational snapshots. No runtime restart or connector refresh is required.
