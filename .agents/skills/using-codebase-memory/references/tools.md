...
| `cbm_get_code_snippet` | Read indexed source for one symbol | `project`, `qualified_name`; optional neighbors | Never starts indexing. Discover the qualified name first. Verify that `source` contains the requested symbol. `source_integrity: bridge_recovered` means the bridge replaced mismatched native line metadata with a bounded range from the verified workspace file; `source_reliable: false` requires direct repository verification. Non-ASCII or whitespace project caveats mean source-unavailable output is not proof of repository absence. |
