...
assert.ok(stateDoc.includes("tests_authenticated=306"));
assert.ok(stateDoc.includes("Latest validated authenticated smoke count: `306`"));
...
assert.ok(readiness.includes("Repository, refreshed connector, and live OAuth21 runtime align at `13 + 85 = 98`"));assert.ok(readiness.includes("| SURF-1 | Connector-visible surface governance | 4/4 |"));
