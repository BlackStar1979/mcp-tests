...
assert.ok(readiness.includes("`SEP-2164`"));
assert.ok(readiness.includes("`SEP-1303`"));
assert.ok(readiness.includes("Production MRTR (`SEP-2322`), capability-adaptive `POL-1A-CONSENT`, `POL-1A-PROMPT-CONTENT`, and `POL-1B` are live accepted on OAuth21/internal"));
assert.ok(readiness.includes("All `6/6` critical policies remain implemented"));
...
assert.equal(workflowState.workflow_progress_markers.next_primary, "pol-1c");
assert.equal(workflowState.workflow_progress_markers.next_secondary, "comp-1a");
...
assert.ok(workflowIndex.includes("`next_primary = pol-1c`"));
assert.ok(workflowIndex.includes("`next_secondary = comp-1a`"));
