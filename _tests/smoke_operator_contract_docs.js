...
assert.ok(readiness.includes("`SEP-2164` and `SEP-1303` are RED-to-GREEN repo-fixed"));
...
assert.equal(workflowState.workflow_progress_markers.current_working_course, "protocol-capability-current-truth-cleanup");
assert.equal(workflowState.workflow_progress_markers.next_primary, "protocol-cleanup-live-validation");
assert.equal(workflowState.workflow_progress_markers.next_secondary, "pol-1a-mrtr-runtime");
...
assert.equal(workflowState.current_runtime_truth.oauth21_3008.server_start_id, "2026-08-18T14:53:56.306Z");
...
assert.ok(workflowIndex.includes("`current_working_course = protocol-capability-current-truth-cleanup`"));
assert.ok(workflowIndex.includes("`next_primary = protocol-cleanup-live-validation`"));
assert.ok(workflowIndex.includes("`next_secondary = pol-1a-mrtr-runtime`"));
