...
assert.ok(stateDoc.includes("repository target connector-visible tools `91`"));
assert.ok(stateDoc.includes("tests_authenticated=278"));
assert.ok(stateDoc.includes("Latest validated authenticated smoke count: `278`"));
...
assert.ok(readiness.includes("Repository target is `13 + 78 = 91`"));
assert.ok(readiness.includes("| PROC-1 | Process execution reliability | 3/4 |"));
...
assert.ok(roadmap.includes("Completed process execution phase A: `PROC-1A`"));
...
assert.equal(workflowState.workflow_progress_markers.current_working_course, "operational-e2e-reliability");
...
assert.ok(workflowIndex.includes("`current_working_course = operational-e2e-reliability`"));
