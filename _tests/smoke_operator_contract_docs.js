...
assert.ok(stateDoc.includes("repository target connector-visible tools `89`"));
assert.ok(stateDoc.includes("tests_authenticated=277"));
assert.ok(stateDoc.includes("Latest validated authenticated smoke count: `277`"));
...
assert.ok(readiness.includes("Repository and live OAuth21 runtime align at `13 + 76 = 89`"));
...
assert.ok(roadmap.includes("Completed process execution acceptance: `PROC-1A`"));
