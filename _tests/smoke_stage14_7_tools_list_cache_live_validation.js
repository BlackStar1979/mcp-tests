...
const expectedStatus = "d1a_d1b_d1c_repo_applied_live_validated_baseline_same_fingerprint";
...
assert.equal(state.tools_list_cache_diagnostics.host_cache_status, "tools_call_after_initialize_without_tools_list_for_current_start");
assert.match(state.tools_list_cache_diagnostics.runtime_restart_required, /^not required/);
assert.match(state.tools_list_cache_diagnostics.connector_refresh_required, /^not required/);
...
assert.ok(canon.includes("observe whether a manual connector-tools refresh causes `tools_list_served`"));
