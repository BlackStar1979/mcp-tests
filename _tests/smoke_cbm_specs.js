...
assert.equal(AUTHORIZED_MCP_TOOL_NAMES.length, 76);
...
assert.equal(toolsSpec.surface_classes.authorized_mcp_tools.tool_count, 76);
assert.equal(toolsSpec.total_mcp_callable_tool_count, 89);
for (const name of CBM_NAMES) {
...
assert.equal(connectorSpec.authenticated_connector.expected_public_plus_authorized_count, 89);
assert.equal(connectorSpec.authenticated_connector.repo_current_expected_tool_count, 89);
assert.equal(connectorSpec.target_connector_topology.active_connectors.authorized.repo_current_expected_tool_count, 89);

assert.equal(serverSpec.server.default_tool_count, 13);
assert.equal(serverSpec.server.public_tool_count, 13);
assert.equal(serverSpec.server.full_tests_authenticated_tool_count, 89);
assert.equal(serverSpec.server.authenticated_tool_count, 89);
assert.equal(serverSpec.runtime_config_spec.env_var_count, 98);
