...
Audit snapshot from `2026-07-03`:

- `293` JavaScript files total in `_tests`
- `223` active scripts currently listed in `run_all_smoke_scripts.json`
...
- last confirmed result from `2026-07-03`: `ok=true`
- last confirmed section counts from that full run: `7` public scripts and `212` authenticated scripts
- current manifest file contains `223` entries, including `_tests/smoke_network.js`; `--skip-network` therefore validates `222` sectioned scripts, not all `223` manifest entries
