...
- `359` JavaScript files total in `_tests`
- `276` active scripts currently listed in `run_all_smoke_scripts.json`
...
- last confirmed section counts from that full run: `7` public scripts and `268` authenticated scripts
- current manifest file contains `275` entries, including `_tests/smoke_network.js`; `--skip-network` therefore validates `274` sectioned scripts, not all `275` manifest entries
