...
Audit snapshot from `2026-07-17`:

- `338` JavaScript files total in `_tests`
- `261` active scripts currently listed in `run_all_smoke_scripts.json`
...
- last confirmed section counts from that full run: `7` public scripts and `253` authenticated scripts
- current manifest file contains `261` entries, including `_tests/smoke_network.js`; `--skip-network` therefore validates `260` sectioned scripts, not all `261` manifest entries
