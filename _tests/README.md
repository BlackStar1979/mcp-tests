...
- `364` JavaScript files total in `_tests`
- `281` active scripts currently listed in `run_all_smoke_scripts.json`
...
- last confirmed section counts from that full run: `7` public scripts and `273` authenticated scripts
- current manifest file contains `281` entries, including `_tests/smoke_network.js`; `--skip-network` therefore validates `280` sectioned scripts, not all `281` manifest entries
