...
assert.equal(evaluatePostAccept({headers:{accept:"application/json"}},{strict:true}).ok,true);
assert.equal(evaluatePostAccept({headers:{accept:"application/json, text/event-stream"}},{strict:true}).ok,true);
assert.equal(evaluatePostAccept({headers:{accept:"text/event-stream"}},{strict:true}).ok,false);
assert.equal(evaluatePostAccept({headers:{accept:"application/json, text/event-stream"}},{strict:true}).mode,"json_only");
...
console.log("smoke_streamable_http_preflight_guards ok");
