...
assert.equal(spec.runtime_instances.oauth21_3008.expected_tool_count, 82);
...
assert.equal(spec.restart_authority.state, "repo_supervisor_authority_live_loaded_on_3008");
assert.equal(spec.restart_authority.required_before_touching_oauth21_3008, false);
...
assert.equal(spec.next_required_repair.status, "complete_for_oauth21_3008");
...
assert.equal(spec.restart_authority.live_supervisor_status.status, "operator_confirmed_supervisor_managed");
assert.equal(spec.restart_authority.live_supervisor_status.controlled_exit_code, 42);

assert.equal(root.spec_refs.runtime_topology, "SERVER_RUNTIME_TOPOLOGY_SPEC.json");
assert.equal(root.runtime_topology_spec.restart_authority_state, "repo_supervisor_authority_live_loaded_on_3008");
...
assert.equal(state.runtime_topology_spec.oauth21_3008_pending_supervisor_migration, false);
