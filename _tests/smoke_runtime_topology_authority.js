...
assert.equal(spec.restart_authority.state, "missing_not_recovered");
...
assert.equal(spec.next_required_repair.status, "pending");

assert.equal(root.spec_refs.runtime_topology, "SERVER_RUNTIME_TOPOLOGY_SPEC.json");
assert.equal(root.runtime_topology_spec.restart_authority_state, "missing_not_recovered");
assert.equal(state.runtime_topology_spec.path, "SERVER_RUNTIME_TOPOLOGY_SPEC.json");
assert.equal(state.runtime_topology_spec.oauth21_3008_blocked_until_authority_restored, true);
