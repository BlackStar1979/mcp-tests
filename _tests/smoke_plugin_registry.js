...
(async () => {
  ...
  for (const name of internalRuntimeTools) {
    const json = await callTool(name, name === "plugin_registry_get" ? { plugin_id: "sample.echo_readonly" } : {});
  ...
  console.log("smoke_plugin_registry ok");
})().catch((error) => {
