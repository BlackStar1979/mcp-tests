...
assert.ok(canon.includes("Latest known full smoke: `node ./_tests/run_all_smokes.js --skip-network = ok_0_40_0_6_199`"));
assert.ok(canon.includes("Latest validated authenticated smoke count: `199`"));
assert.ok(canon.includes("S12 connector migration dry-run plan / no refresh"));
assert.ok(index.includes("Authenticated smoke count: `199`."));
assert.ok(index.includes("s11_connector_migration_readiness_coexistence_boundary.md"));
assert.ok(index.includes("S12 connector migration dry-run plan / no refresh"));
