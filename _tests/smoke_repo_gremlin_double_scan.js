...
const stressWorkflow = read("_workflow", "historical", "progress_state_dependent_validators", "_tests", "stress_workflow.js");
const preflightGuard = read("_tests", "smoke_preflight_control_plane_guard.js");
const stage14_2Guard = read("_tests", "smoke_workbench_debt_cleanup.js");
const streamableGuard = read("_tests", "smoke_streamable_http_workflow_plan.js");
...
const oldPackageKey = "current_" + "work_" + "package";
const currentPackageIdPin = "state." + oldPackageKey + "." + "id";
const currentPackageStatusPin = "state." + oldPackageKey + "." + "status";
...
assert.equal(streamableGuard.includes('typeof state.' + oldPackageKey + '.id === "string"'), false);

assert.equal(state.schema_version, "workflow-state-spec-map-v2");
assert.ok(!Object.hasOwn(state, "stage14"));
assert.ok(manifest.includes("_tests/smoke_repo_gremlin_double_scan.js"));
assert.ok(index.includes("stage14_2b_repo_gremlin_double_scan.md"));

console.log("smoke_repo_gremlin_double_scan ok");
