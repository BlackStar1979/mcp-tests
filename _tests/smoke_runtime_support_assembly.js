...
check("src/runtime/registry_context_assembly.js");
...
const registryContext=read("src/runtime/registry_context_assembly.js");
...
for(const required of ["createRuntimeRegistryContextFactory","createAuditLogger","createDocumentRuntimeContext","function createRuntimeSupportAssembly","function toolsList","registryContext({ label: \"runtime-tools-list\" })"]){assert.ok(support.includes(required),required);}
for(const required of ["buildCoreToolDescriptors","createStaticToolRegistry","buildRegistryPolicyReadModel","snapshotRegistryForDiff","function createRuntimeRegistryContextFactory"]){assert.ok(registryContext.includes(required),required);}
const mod=require("../src/runtime/runtime_support_assembly");
assert.equal(typeof mod.createRuntimeSupportAssembly,"function");
const registryMod=require("../src/runtime/registry_context_assembly");
assert.equal(typeof registryMod.createRuntimeRegistryContextFactory,"function");
console.log("smoke_runtime_support_assembly ok");
