...
function expectedInnerResultCount() {
  ...
  const noPollutionSkipped = scripts.includes("_tests/smoke_harness_no_pollution_guard.js") ? 1 : 0;
  const networkSkipped = scripts.includes("_tests/smoke_network.js") ? 1 : 0;
...
(async () => {
  ...
  const innerPort = process.env.MCP_TEST_NO_POLLUTION_INNER_PORT
    ...
    : await getFreePort();
  const innerInternalPort = process.env.MCP_TEST_NO_POLLUTION_INNER_INTERNAL_PORT
    ? Number(process.env.MCP_TEST_NO_POLLUTION_INNER_INTERNAL_PORT)
    : await getFreePort();
  ...
  const child = spawnSync(
    ...
    ["_tests/run_all_smokes.js", "--skip-network"],
    {
      ...
      env: {
        ...
        // Explicit internal port so run_all_smokes does not derive an invalid
        // port when the chosen shared port is already near the TCP upper bound.
        MCP_TEST_SMOKE_INTERNAL_PORT: String(innerInternalPort),
      },
      encoding: "utf8",
