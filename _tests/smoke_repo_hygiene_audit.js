...
assert.equal(totalJsFiles.length, 334);
assert.equal(smokeScripts.length, 257);
...
assert.ok(testsReadme.includes("`334` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`257` active scripts currently listed in `run_all_smoke_scripts.json`"));
