...
assert.equal(totalJsFiles.length, 379);
assert.equal(smokeScripts.length, 295);
...
assert.ok(testsReadme.includes("`379` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`295` active scripts currently listed in `run_all_smoke_scripts.json`"));
