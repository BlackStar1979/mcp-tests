...
assert.equal(totalJsFiles.length, 404);
assert.equal(smokeScripts.length, 319);
...
assert.ok(testsReadme.includes("`404` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`319` active scripts currently listed in `run_all_smoke_scripts.json`"));
