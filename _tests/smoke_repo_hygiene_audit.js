...
assert.equal(totalJsFiles.length, 375);
assert.equal(smokeScripts.length, 291);
...
assert.ok(testsReadme.includes("`375` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`291` active scripts currently listed in `run_all_smoke_scripts.json`"));
