...
assert.equal(totalJsFiles.length, 380);
assert.equal(smokeScripts.length, 296);
...
assert.ok(testsReadme.includes("`380` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`296` active scripts currently listed in `run_all_smoke_scripts.json`"));
