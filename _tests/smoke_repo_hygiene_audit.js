...
assert.equal(totalJsFiles.length, 399);
assert.equal(smokeScripts.length, 314);
...
assert.ok(testsReadme.includes("`399` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`314` active scripts currently listed in `run_all_smoke_scripts.json`"));
