...
assert.equal(totalJsFiles.length, 386);
assert.equal(smokeScripts.length, 302);
...
assert.ok(testsReadme.includes("`386` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`302` active scripts currently listed in `run_all_smoke_scripts.json`"));
