...
assert.equal(totalJsFiles.length, 337);
assert.equal(smokeScripts.length, 260);
...
assert.ok(testsReadme.includes("`337` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`260` active scripts currently listed in `run_all_smoke_scripts.json`"));
