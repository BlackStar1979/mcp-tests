...
assert.equal(totalJsFiles.length, 335);
assert.equal(smokeScripts.length, 258);
...
assert.ok(testsReadme.includes("`335` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`258` active scripts currently listed in `run_all_smoke_scripts.json`"));
