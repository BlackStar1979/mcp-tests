...
assert.equal(totalJsFiles.length, 336);
assert.equal(smokeScripts.length, 259);
...
assert.ok(testsReadme.includes("`336` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`259` active scripts currently listed in `run_all_smoke_scripts.json`"));
