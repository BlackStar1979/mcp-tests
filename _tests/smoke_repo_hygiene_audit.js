...
assert.equal(totalJsFiles.length, 373);
assert.equal(smokeScripts.length, 289);
...
assert.ok(testsReadme.includes("`373` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`289` active scripts currently listed in `run_all_smoke_scripts.json`"));
