...
assert.equal(totalJsFiles.length, 359);
assert.equal(smokeScripts.length, 276);
...
assert.ok(testsReadme.includes("`359` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`276` active scripts currently listed in `run_all_smoke_scripts.json`"));
