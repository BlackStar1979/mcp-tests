...
assert.equal(totalJsFiles.length, 372);
assert.equal(smokeScripts.length, 288);
...
assert.ok(testsReadme.includes("`372` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`288` active scripts currently listed in `run_all_smoke_scripts.json`"));
