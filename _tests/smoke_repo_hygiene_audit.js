...
assert.equal(totalJsFiles.length, 398);
assert.equal(smokeScripts.length, 313);
...
assert.ok(testsReadme.includes("`398` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`313` active scripts currently listed in `run_all_smoke_scripts.json`"));
