...
assert.equal(totalJsFiles.length, 358);
assert.equal(smokeScripts.length, 275);
...
assert.ok(testsReadme.includes("`358` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`275` active scripts currently listed in `run_all_smoke_scripts.json`"));
