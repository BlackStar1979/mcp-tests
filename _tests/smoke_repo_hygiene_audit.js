...
assert.equal(totalJsFiles.length, 383);
assert.equal(smokeScripts.length, 299);
...
assert.ok(testsReadme.includes("`383` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`299` active scripts currently listed in `run_all_smoke_scripts.json`"));
