...
assert.equal(totalJsFiles.length, 364);
assert.equal(smokeScripts.length, 281);
...
assert.ok(testsReadme.includes("`364` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`281` active scripts currently listed in `run_all_smoke_scripts.json`"));
