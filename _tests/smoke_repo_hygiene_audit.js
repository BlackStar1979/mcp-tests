...
assert.equal(totalJsFiles.length, 330);
assert.equal(smokeScripts.length, 252);
...
assert.ok(testsReadme.includes("`330` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`252` active scripts currently listed in `run_all_smoke_scripts.json`"));
