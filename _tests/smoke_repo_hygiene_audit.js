...
assert.equal(totalJsFiles.length, 361);
assert.equal(smokeScripts.length, 278);
...
assert.ok(testsReadme.includes("`361` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`278` active scripts currently listed in `run_all_smoke_scripts.json`"));
