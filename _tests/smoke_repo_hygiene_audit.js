...
assert.equal(totalJsFiles.length, 396);
assert.equal(smokeScripts.length, 312);
...
assert.ok(testsReadme.includes("`396` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`312` active scripts currently listed in `run_all_smoke_scripts.json`"));
