...
assert.equal(totalJsFiles.length, 378);
assert.equal(smokeScripts.length, 294);
...
assert.ok(testsReadme.includes("`378` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`294` active scripts currently listed in `run_all_smoke_scripts.json`"));
