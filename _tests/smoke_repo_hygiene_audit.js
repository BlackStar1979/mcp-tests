...
assert.equal(totalJsFiles.length, 360);
assert.equal(smokeScripts.length, 277);
...
assert.ok(testsReadme.includes("`360` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`277` active scripts currently listed in `run_all_smoke_scripts.json`"));
