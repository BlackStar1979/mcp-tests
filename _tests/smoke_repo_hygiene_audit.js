...
assert.equal(totalJsFiles.length, 324);
assert.equal(smokeScripts.length, 246);
...
assert.ok(testsReadme.includes("`324` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`246` active scripts currently listed in `run_all_smoke_scripts.json`"));
