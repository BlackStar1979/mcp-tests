...
assert.equal(totalJsFiles.length, 329);
assert.equal(smokeScripts.length, 251);
...
assert.ok(testsReadme.includes("`329` JavaScript files total in `_tests`"));
assert.ok(testsReadme.includes("`251` active scripts currently listed in `run_all_smoke_scripts.json`"));
