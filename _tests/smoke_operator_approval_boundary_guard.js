...
function readinessReport(overrides = {}) {
  return { ...buildEnforcementApplyReadinessReport({
    ...
    authorizedMatrix: { tool_count: 82, blocked_count: 0, runtime_enforcement_changed: false, allow_deny_behavior_changed: false, connector_visible_schema_changed: false },
    authorizedEvaluation: { would_deny_count: 0, would_allow_count: 82, runtime_enforcement_changed: false, allow_deny_behavior_changed: false, connector_visible_schema_changed: false },
    receiptSet: { receipt_count: 82, denied_receipt_count: 0, raw_arguments_included: false, runtime_audit_event_emitted: false },
...
(() => {
  ...
  assert.equal(noMarker.schema_version, "runtime-enforcement-operator-approval-boundary-guard-v1");
  ...
  assert.ok(noMarker.blockers.includes("plan_only_boundary_guard_is_no_apply_even_with_valid_marker"));
  ...
  assert.deepEqual(validStillNoApply.blockers, ["plan_only_boundary_guard_is_no_apply_even_with_valid_marker"]);
  ...
  console.log("smoke_operator_approval_boundary_guard ok");
})();
