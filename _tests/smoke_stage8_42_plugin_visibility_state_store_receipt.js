...
const quarantineReceipt = buildStateStoreTransactionReceipt({ plan: quarantinePlan, stage: "8.42", operation: "quarantine_preview" });
...
const failureReceipt = buildStateStoreTransactionReceipt({ plan: missingMetadataPlan, stage: "8.42", operation: "failure_preview" });
...
assert.ok(appliedResult.errors.includes("Stage 8 / Step 42 receipt must not record applied transaction"));
...
console.log("smoke_stage8_42_plugin_visibility_state_store_receipt ok");
