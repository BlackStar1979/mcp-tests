...
assert.ok(index.includes("Runtime and authenticated connector expose the unchanged `98`-tool surface with combined fingerprint"));
...
assert.ok(index.includes("connector refresh is not required"));
...
assert.ok(index.includes("1. Execute `POL-1B` against the four `partial` non-critical policies using current code/runtime evidence before adding new behavior."));
...
assert.ok(canon.includes("Preserve production MRTR, live-accepted capability-adaptive `POL-1A-CONSENT`, live-accepted `POL-1A-PROMPT-CONTENT`, `OPS-1`, live Tasks/Trace/Artifacts/CIMD"));
