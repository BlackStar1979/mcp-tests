...
assert.ok(index.includes("Current repo/runtime note: profile `tests`; live OAuth21 `3008` runs the capability-adaptive authorization source"));
assert.ok(index.includes("Runtime and authenticated connector expose `98` tools with unchanged combined fingerprint"));
...
assert.ok(index.includes("`restart_required_now = false`"));
...
assert.ok(index.includes("1. Execute `POL-1A-PROMPT` for explicit untrusted-content classification and instruction-promotion boundaries"));
assert.ok(index.includes("capability-adaptive `POL-1A-CONSENT` authorization is live accepted"));
...
assert.ok(canon.includes("Preserve production MRTR, live-accepted capability-adaptive `POL-1A-CONSENT`, `OPS-1`, live Tasks/Trace/Artifacts/CIMD"));
