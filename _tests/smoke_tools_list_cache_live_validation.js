...
const expectedCurrentStatus = "live_authenticated_tools_list_78_matches_repo_expected_78_connector_ui_visibility_unverified";
const expectedCurrentFingerprint = "3248a1bef9b94318";
const expectedCurrentHash = "b8fadaa824febfbb";
...
assert.equal(c.tool_count, 78);
assert.equal(c.repo_current_expected_tool_count, 78);
...
assert.ok(index.includes("repo truth expects `78` authenticated tools"));
assert.ok(index.includes("live authenticated runtime + `tools/list` now confirms 78 tools"));
assert.ok(index.includes("visible-tool enumeration is still not independently re-verified"));
assert.ok(canon.includes("Repo current authenticated tool target after the bounded modular unsafe-governance batch: `78`"));
assert.ok(canon.includes("Live OAuth21 `3008` is reconciled to the authenticated `/mcp` tool count `78`"));
