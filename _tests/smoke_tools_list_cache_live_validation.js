...
assert.ok(index.includes("The modular standards cleanup remains accepted"));
assert.ok(index.includes("durable restart recovery are green"));
...
assert.ok(index.includes("production `MRTR` (`7 + 310`) are live-accepted"));
assert.ok(index.includes("MRTR is loaded-dormant until policy emits `mrtr_requirement`; no connector refresh is pending."));
...
assert.ok(index.includes("1. `POL-1A-CONSENT`: bind high-risk operations to server-verifiable human consent carried through production MRTR."));
assert.ok(index.includes("2. `POL-1A-PROMPT`: add explicit untrusted-content classification and instruction-promotion boundaries."));
...
assert.ok(canon.includes("Preserve accepted production MRTR, `OPS-1`, live Tasks/Trace/Artifacts/CIMD"));
