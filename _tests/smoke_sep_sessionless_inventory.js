...
assert.ok(inventory.recommended_next.some((item) => item.includes("Repo hygiene triage is complete")));
assert.ok(inventory.recommended_next.some((item) => item.includes("no safe narrow standalone push exists yet")));
assert.ok(inventory.recommended_next.some((item) => item.includes("Rename-normalization package scoping is complete")));
assert.ok(inventory.recommended_next.some((item) => item.includes("execute one coherent `_tests`/`_workflow` rename-normalization migration package before any push attempt")));
...
assert.equal(inventory.repo_hygiene_commit_scope_triage.push_safe_now, false);
assert.equal(inventory.repo_hygiene_commit_scope_triage.next_step, "commit_scope_isolation_before_any_push_attempt");
...
assert.equal(inventory.repo_hygiene_commit_scope_isolation.safe_narrow_commit_available_now, false);
assert.equal(inventory.repo_hygiene_commit_scope_isolation.push_safe_now, false);
assert.equal(inventory.repo_hygiene_commit_scope_isolation.next_step, "prepare_coherent_tests_workflow_rename_normalization_migration_package");
...
assert.equal(inventory.tests_workflow_rename_normalization_package.push_safe_now, false);
assert.equal(inventory.tests_workflow_rename_normalization_package.next_step, "execute_coherent_tests_workflow_rename_normalization_migration_package");
