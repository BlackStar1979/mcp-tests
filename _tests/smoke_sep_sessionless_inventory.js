...
assert.equal(ledger.get("initialize_handshake").checklist.find((item) => item.item === "record real client evidence that some connectors still call initialize after successful OAuth").status, "done");
...
assert.equal(ledger.get("session_store").target_lifecycle, "removed");
assert.equal(ledger.get("session_store").checklist.find((item) => item.item === "record bounded SessionStore closeout").status, "done");
assert.equal(ledger.get("session_store").checklist.find((item) => item.item === "retire residual McpSession and outbound pending-request helpers").status, "done");
...
assert.equal(ledger.get("roots_sampling_logging_deprecation").checklist.find((item) => item.item === "retire deprecated classic Sampling helper implementation").status, "done");
...
assert.equal(ledger.get("feature_lifecycle_deprecation_policy").implementation_status, "done");
assert.match(ledger.get("feature_lifecycle_deprecation_policy").repo_current_model, /lifecycle\/status\/migration\/checklist/);
assert.equal(ledger.get("feature_lifecycle_deprecation_policy").checklist.find((item) => item.item === "record lifecycle-governance closeout").status, "done");
...
assert.ok(inventory.recommended_next.some((item) => item.includes("Earlier local-helper retention records are historical")));
...
assert.equal(inventory.active_target_contract.deprecated_sampling_helper_retirement_closeout_record, "_workflow/operator_decisions/deprecated_sampling_helper_retirement_closeout.md");
assert.equal(inventory.active_target_contract.deprecated_sampling_runtime_active, false);
...
assert.match(ledger.get("session_store").repo_current_model, /removed from the active repo/);
...
assert.equal(ledger.get("resumable_sse_last_event_id").implementation_status, "done");
...
assert.equal(ledger.get("resumable_sse_last_event_id").checklist.find((item) => item.item === "remove residual replay helper files from the active repo").status, "done");
