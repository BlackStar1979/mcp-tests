...
assert.equal(state.workflow_progress_markers.current_working_course, "memory-embedding-runtime-activation-gate");
assert.equal(state.workflow_progress_markers.next_primary, "mem-1-live-provider-activation-and-quality-proof");
assert.equal(state.workflow_progress_markers.next_secondary, "mem-1-credential-provision-and-quality-proof");
assert.ok(index.includes("`current_working_course = memory-embedding-runtime-activation-gate`"));
assert.ok(index.includes("`next_primary = mem-1-live-provider-activation-and-quality-proof`"));
assert.ok(index.includes("`next_secondary = mem-1-credential-provision-and-quality-proof`"));
assert.ok(canon.includes("`current_working_course = memory-embedding-runtime-activation-gate`"));
assert.ok(canon.includes("`next_primary = mem-1-live-provider-activation-and-quality-proof`"));
assert.ok(canon.includes("`next_secondary = mem-1-credential-provision-and-quality-proof`"));
