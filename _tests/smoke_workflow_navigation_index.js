...
assert.ok(index.includes("`next_secondary = pol-1a-prompt-content`"));
...
assert.ok(canon.includes("`next_secondary = pol-1a-prompt-content`"));
assert.ok(canon.includes("`SERVER_PROTOCOL_CAPABILITY_SPEC.json`"));
assert.ok(canon.includes("MRTR (`SEP-2322`) is production-loaded and live accepted"));
assert.ok(canon.includes("start `POL-1A-CONSENT` on the live production MRTR boundary"));
