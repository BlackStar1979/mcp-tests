...
assert.equal(state.workflow_progress_markers.current_working_course, "operational-e2e-quality");
assert.equal(state.workflow_progress_markers.next_primary, "ops-1a-operational-e2e-coverage-matrix");
assert.equal(state.workflow_progress_markers.next_secondary, "comp-1a-on-fresh-external-client-traffic");
assert.ok(index.includes("`current_working_course = operational-e2e-quality`"));
assert.ok(index.includes("`next_primary = ops-1a-operational-e2e-coverage-matrix`"));
assert.ok(index.includes("`next_secondary = comp-1a-on-fresh-external-client-traffic`"));
