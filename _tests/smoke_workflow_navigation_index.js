...
assert.equal(state.workflow_progress_markers.current_working_course, "protocol-compatibility-evidence-gate");
assert.equal(state.workflow_progress_markers.next_primary, "comp-1a-on-fresh-external-client-traffic");
assert.equal(state.workflow_progress_markers.next_secondary, "doc-2a-on-demonstrated-orientation-gap");
assert.ok(index.includes("`current_working_course = protocol-compatibility-evidence-gate`"));
assert.ok(index.includes("`next_primary = comp-1a-on-fresh-external-client-traffic`"));
assert.ok(index.includes("`next_secondary = doc-2a-on-demonstrated-orientation-gap`"));
assert.ok(canon.includes("`current_working_course = protocol-compatibility-evidence-gate`"));
assert.ok(canon.includes("`next_primary = comp-1a-on-fresh-external-client-traffic`"));
assert.ok(canon.includes("`next_secondary = doc-2a-on-demonstrated-orientation-gap`"));
