...
assert.ok(index.includes("Execute `POL-1A-DLP`: add one centralized, fail-closed model-output validation/DLP boundary"));
assert.ok(index.includes("`POL-1A-DLP` — centralized model-output validation/DLP boundary"));
...
assert.ok(index.includes("Re-run `COMP-1A` only when real Codex client-entry traffic newer than `0.148.0-alpha.9` exists; run `COMP-1B` only if that evidence materially changes initialize-retirement."));
...
assert.equal(state.workflow_progress_markers.current_working_course, "policy-coverage-critical-gap-closure");
assert.equal(state.workflow_progress_markers.next_primary, "pol-1a-output-dlp-boundary");
assert.equal(state.workflow_progress_markers.next_secondary, "pol-1a-consent-boundary");
assert.ok(index.includes("`current_working_course = policy-coverage-critical-gap-closure`"));
...
assert.ok(canon.includes("`next_primary = pol-1a-output-dlp-boundary`"));
assert.ok(canon.includes("`next_secondary = pol-1a-consent-boundary`"));
assert.ok(canon.includes("Execute `POL-1A-DLP`"));
