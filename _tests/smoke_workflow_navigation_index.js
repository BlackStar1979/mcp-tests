...
assert.equal(state.workflow_progress_markers.next_primary, "pol-1a-prompt-content");
assert.equal(state.workflow_progress_markers.next_secondary, "pol-1b");
assert.ok(index.includes("`current_working_course = protocol-capability-module-convergence`"));
assert.ok(index.includes("`next_primary = pol-1a-prompt-content`"));
assert.ok(index.includes("`next_secondary = pol-1b`"));
assert.ok(canon.includes("`current_working_course = protocol-capability-module-convergence`"));
assert.ok(canon.includes("`next_primary = pol-1a-prompt-content`"));
assert.ok(canon.includes("`next_secondary = pol-1b`"));
