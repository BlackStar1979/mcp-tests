...
(async () => {
  ...
  const missing = baseRuntimeStatus();
  delete missing.stage_status;
  const missingResult = validateAgainstSchema(missing, RUNTIME_STATUS_OUTPUT_SCHEMA);
  assert.equal(missingResult.success, false);
  assert.ok(missingResult.issues.some((issue) => issue.path === "$.stage_status" && issue.message.includes("required")));
  ...
  console.log("smoke_stage8_21_output_schema_guard ok");
})().catch((error) => {
