...
(async () => {
  ...
  const missingCompatibility = { ...payload };
  delete missingCompatibility.compatibility_label;
  const missingCompatResult = validateAgainstSchema(missingCompatibility, OBSERVABILITY_STATUS_OUTPUT_SCHEMA);
  ...
  const missingStage = { ...payload };
  delete missingStage.stage;
  const missingResult = validateAgainstSchema(missingStage, OBSERVABILITY_STATUS_OUTPUT_SCHEMA);
  ...
  console.log("smoke_observability_output_schema ok");
})().catch((error) => {
