...
const optionalTools = [];
const support = createRuntimeSupportAssembly({auditLogPath:path.join(ROOT,"_logs",".authorized-split-test.jsonl"),auditVersion:AUDIT_VERSION,serverName:SERVER_NAME,serverVersion:SERVER_VERSION,connectorShapeVersion:CONNECTOR_SHAPE_VERSION,docs:[],publicBaseUrl:"http://127.0.0.1:3008",maxFetchTextChars:2500,outputMode:"structured",optionalTools,rootDir:ROOT});
const tools=loadOptionalTools({profile:"internal",authPolicy:{mode:"access",requiresAuth:true},serverProfileConfig:{surface:testsProfile.surfaces.authenticated},createRuntimeStatusTool:()=>createTestMcpRuntimeStatusTool(()=>({status:"ok"})),createObservabilityStatusTool:()=>createObservabilityStatusTool({auditLogPath:"_logs/.mcp-tests-audit.jsonl"}),createRuntimeRegistryContext:(label)=>support.registryContext({label})});
