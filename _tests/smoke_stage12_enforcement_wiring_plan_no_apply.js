...
(() => {
  ...
  assert.ok(plan.blockers.includes("Stage 12.2 is plan-only; apply is intentionally disabled in this stage"));
  ...
  assert.deepEqual(approvedStillNoApply.blockers, ["Stage 12.2 is plan-only; apply is intentionally disabled in this stage"]);

  const handlerSource = fs.readFileSync(path.join(ROOT, "src/runtime/tools_call_handler.js"), "utf8");
  assert.equal(handlerSource.includes("policy_preflight"), false, "tools_call_handler must not be wired to policy preflight in Stage 12.2");
  ...
  console.log("smoke_stage12_enforcement_wiring_plan_no_apply ok");
})();
