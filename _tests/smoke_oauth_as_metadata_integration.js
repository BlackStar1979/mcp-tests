...
(async()=>{
 ...
 const child=spawn(process.execPath,["server.js","--auth","oauth","--port",String(port)],{cwd:ROOT,env:withHermeticServerControlEnv({...process.env,MCP_TEST_PUBLIC_BASE_URL:publicBaseUrl,MCP_TEST_OAUTH_ISSUER:issuer,MCP_TEST_OAUTH_AUDIENCE:publicBaseUrl,MCP_TEST_OAUTH_HS256_SECRET_FILE:secretFile,MCP_TEST_OAUTH_AS_METADATA_FILE:metadataFile,MCP_TEST_FS_ROOT:path.join(ROOT,"_public_sandbox")},path.join(tmp,"control")),stdio:["ignore","pipe","pipe"]});
 ...
 console.log("smoke_oauth_as_metadata_integration ok");
