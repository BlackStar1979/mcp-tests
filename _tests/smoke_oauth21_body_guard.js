...
async function postWrongContentType(issuer,path,contentType,bodyText){const r=await fetch(`${issuer}${path}`,{method:"POST",headers:{"content-type":contentType},body:bodyText});assert.equal(r.status,400,`${path} ${contentType}`);assert.ok((r.headers.get("content-type")||"").includes("application/json"), path);const body=await r.json();assert.equal(body.error,"invalid_request",`${path} ${contentType}`);}

(async()=>{const port=await freePort();const tmp=fs.mkdtempSync(path.join(os.tmpdir(),"mcp-tests-oauth21-body-"));const cfg=path.join(tmp,"oauth.json");const operatorSecret="stage12-oauth21-operator-secret";const issuer=`http://127.0.0.1:${port}`;fs.writeFileSync(cfg,JSON.stringify({operator_secret:operatorSecret,issuer}),"utf8");const child=spawn(process.execPath,["server.js","--profile","tests","--auth","oauth21","--oauth-secret-file",cfg,"--port",String(port)],{cwd:ROOT,env:cleanEnv({MCP_TEST_FS_ROOT:path.join(ROOT,"_public_sandbox"),MCP_TEST_PUBLIC_BASE_URL:issuer}),stdio:["ignore","pipe","pipe"]});let output="";child.stdout.on("data",d=>{output+=String(d);});child.stderr.on("data",d=>{output+=String(d);});try{const health=await waitHealth(port);assert.equal(health.auth.mode,"oauth21");for(const route of ["/register","/token","/revoke","/oauth/operator-login"]){await postMalformed(issuer,route);const stillHealthy=await fetch(`${issuer}/healthz`);assert.equal(stillHealthy.status,200, route);}await postWrongContentType(issuer,"/register","application/x-www-form-urlencoded","redirect_uris=http%3A%2F%2Flocalhost%2Fcb");await postWrongContentType(issuer,"/token","application/json",JSON.stringify({grant_type:"refresh_token"}));await postWrongContentType(issuer,"/revoke","application/json",JSON.stringify({token:"x",client_id:"y"}));await postWrongContentType(issuer,"/oauth/operator-login","application/json",JSON.stringify({pid:"x",password:"y"}));await postWrongContentType(issuer,"/register","text/application/json","{}");await postWrongContentType(issuer,"/token","application/x-www-form-urlencodedevil","grant_type=refresh_token");console.log("smoke_oauth21_body_guard ok");}catch(e){e.message+=output?`
server output:
