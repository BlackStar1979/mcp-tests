...
const OLD_STAGE_ALLOWED = new Set([
  path.normalize("_tests/smoke_status_current.js"),
  path.normalize("_tests/smoke_no_stale_stage_strings.js"),
]);
...
function walk(dir, hits = [], currentHits = []) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      if (path.relative(path.resolve(__dirname, ".."), full).startsWith(path.join("_tests", "archive"))) {
        continue;
      }
...
console.log("smoke_no_stale_stage_strings ok");
