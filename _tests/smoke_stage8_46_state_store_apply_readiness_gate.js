...
(async () => {
  ...
  assert.ok(denied.denied_reasons.includes("Stage 8 / Step 46 is readiness-only; apply remains disabled"));
  ...
  assert.ok(futureReady.denied_reasons.includes("Stage 8 / Step 46 is readiness-only; apply remains disabled"));
  ...
  assert.ok(tamperedResult.errors.includes("apply_allowed_now must be false in Stage 8 / Step 46"));
  assert.ok(tamperedResult.errors.includes("gate_hash mismatch"));

  console.log("smoke_stage8_46_state_store_apply_readiness_gate ok");
})().catch((error) => {
