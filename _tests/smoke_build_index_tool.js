...
(async () => {
  ...
  try {
    ...
    try {
      ...
      const freshWorkflow = freshStatus.knowledge_summary.workflow_summary;
      assert.equal(freshWorkflow.workflow_markers.next_primary, "comp-1a-on-fresh-external-client-traffic");
      assert.equal(freshWorkflow.workflow_markers.next_secondary, "bounded-doc-orientation-maintenance");
      assert.equal(freshWorkflow.readiness.components[0].id, "DOC-1");
      assert.equal(freshWorkflow.readiness.components[0].main_blocker, "Needs direct workflow extraction");
      assert.equal(freshWorkflow.readiness.components[0].default_next_package, "Add workflow summary");
      assert.equal(freshWorkflow.roadmap.items[0].priority, "P0");
      assert.equal(freshWorkflow.roadmap.items[0].current_action, "Wait for newer traffic");
      assert.equal(freshWorkflow.documentation_gaps.length, 0);
